...
const DATA_DIR = path.resolve(process.env.SMARTDESK_DATA_DIR || path.resolve(process.cwd(), "data"));
const EXPORTS_DIR = path.resolve(process.env.SMARTDESK_EXPORTS_DIR || path.resolve(process.cwd(), "public", "exports"));
...
function localizeDecisionCenterPayload(payload, language = "it") {
  if (!payload || !["en", "de"].includes(language)) return payload;
  const walk = (value) => {
...
function localizeOperationalReportPayload(report, language = "it") {
  if (!report || !["en", "de"].includes(language)) return report;
  return {
    ...report,
...
function localizeCenterHealthPayload(health, language = "it") {
  if (!health || !["en", "de"].includes(language)) return health;
  return {
    ...health,
...
class DesktopMirrorService {
  ...
  async commitRepositorySnapshots(changes = []) {
    ...
    const staged = changes.every(({ repository, payload }) => this.persistenceAdapter.stageWrite(
      repository.collectionName,
      payload,
      repository.revision,
      (revision) => repository.setRevision(revision)
    ));
    if (staged) {
      changes.forEach(({ repository, payload }) => repository.write(payload, {
        skipLegacySync: true,
  ...
  async init() {
    ...
    await this.persistenceAdapter.runWithWriteTracking(async (context) => {
      try {
        await seedRuntime();
        await this.persistenceAdapter.flushTrackedWrites(context);
      } finally {
        this.persistenceAdapter.releaseTrackedWrites(context);
      }
    });
  }
