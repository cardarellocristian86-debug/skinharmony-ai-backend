  ...
  function openStaffDialog(member = null) {
    openDialog({
      ...
      fields: `
        ...
        ${dialogField({ label: t("dialogs.target"), name: "targetProgress", type: "number", value: member?.targetProgress || 0, help: currentLanguage() === "en" ? "Monthly target/progress reference." : "Target o avanzamento mensile." })}
        ${dialogField({ label: currentLanguage() === "en" ? "Hourly operator cost" : "Costo orario operatore", name: "hourlyCostCents", type: "number", value: member?.hourlyCostCents || member?.hourlyCost || 0, help: currentLanguage() === "en" ? "Cost per working hour in cents. Example: 1800 = 18.00 euros/hour." : "Costo per ora lavorata in centesimi. Esempio: 1800 = 18,00 euro/ora." })}
        ${dialogField({ label: currentLanguage() === "en" ? "Net monthly salary" : "Stipendio netto mensile", name: "netSalaryCents", type: "number", value: member?.netSalaryCents || member?.monthlyNetSalaryCents || 0, help: currentLanguage() === "en" ? "Context only; gross/payroll cost is used for center cost." : "Solo contesto; per il costo centro conta il lordo/costo azienda." })}
        ${dialogField({ label: currentLanguage() === "en" ? "Gross monthly payroll cost" : "Costo lordo mensile azienda", name: "grossSalaryCents", type: "number", value: member?.grossSalaryCents || member?.monthlyGrossSalaryCents || member?.payrollCostCents || 0, help: currentLanguage() === "en" ? "Monthly company cost used in Gold cost-per-minute." : "Costo azienda mensile usato nel costo minuto Gold." })}
      `
    });
  }
