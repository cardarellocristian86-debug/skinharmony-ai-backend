...
function runtimeShardDescriptorWithinLimits(descriptor) {
  return Number.isInteger(descriptor?.compressed_bytes)
    ...
    && /^[a-f0-9]{64}$/.test(String(descriptor?.compressed_sha256 || ""))
    ...
    && /^[a-f0-9]{64}$/.test(String(descriptor?.uncompressed_sha256 || ""))
...
function runtimeCatalogTopology(catalog) {
...
function runtimeBranchTopologyProjection(branches) {
...
function verifyRuntimeManifestArtifacts(
  manifest,
  manifestPath,
  { verifyContent = false } = {},
) {
  ...
  let checkedShards = 0;
  let contentVerifiedShards = 0;
  let compressedBytes = 0;
  let uncompressedBytes = 0;
  let peakCompressedShardBytes = 0;
  let peakUncompressedShardBytes = 0;
  let manifestRoot;
  try {
    manifestRoot = fs.realpathSync(path.dirname(manifestPath));
  } catch (error) {
    return {
      ok: false,
      errors: [`runtime_manifest_root_unavailable:${error.code || error.message}`],
      checked_shards: 0,
      unchecked_shards: descriptors.length,
      content_verified_shards: 0,
      deferred_content_shards: descriptors.length,
      compressed_bytes: 0,
      uncompressed_bytes: 0,
      peak_compressed_shard_bytes: 0,
      peak_uncompressed_shard_bytes: 0,
      parse_mode: verifyContent
        ? "full_hash_gunzip_and_json_deferred"
        : "manifest_descriptor_and_file_stat_only",
    };
  }
  for (const descriptor of descriptors) {
    ...
    try {
      ...
      if (
        artifactRealPath === manifestRoot
        || !artifactRealPath.startsWith(`${manifestRoot}${path.sep}`)
      ) {
        errors.push(`unsafe_shard_realpath:${descriptor?.branch_id || "unknown"}.${descriptor?.subbranch_id || "unknown"}`);
        continue;
      }
      const stat = fs.statSync(artifactRealPath);
      if (!stat.isFile() || stat.size !== descriptor.compressed_bytes) {
        errors.push(`compressed_shard_size_mismatch:${descriptor.branch_id}.${descriptor.subbranch_id}`);
        continue;
      }
      compressedBytes += stat.size;
      uncompressedBytes += descriptor.uncompressed_bytes;
      peakCompressedShardBytes = Math.max(peakCompressedShardBytes, stat.size);
      peakUncompressedShardBytes = Math.max(
        peakUncompressedShardBytes,
        descriptor.uncompressed_bytes,
      );
      if (!verifyContent) continue;
      const compressed = fs.readFileSync(artifactRealPath);
      if (byteHash(compressed) !== descriptor.compressed_sha256) {
  ...
  return {
    ...
    content_verified_shards: contentVerifiedShards,
    deferred_content_shards: Math.max(
      0,
      descriptors.length - contentVerifiedShards,
    ),
    compressed_bytes: compressedBytes,
    uncompressed_bytes: uncompressedBytes,
    peak_compressed_shard_bytes: peakCompressedShardBytes,
    peak_uncompressed_shard_bytes: peakUncompressedShardBytes,
    parse_mode: verifyContent
      ? "sequential_hash_and_gunzip_without_json_parse"
      : "manifest_descriptor_and_file_stat_only",
  };
}
...
function validateRuntimeManifest(
  ...
  const expectedTopology = runtimeCatalogTopology(catalog);
  const catalogTopologyProjection = runtimeBranchTopologyProjection(catalog.branches);
  const sourceTopologyProjection = runtimeBranchTopologyProjection(catalog.source_catalog?.branches);
  ...
  if (root.catalog_runtime_sha256 !== root.runtime_loader_sha256) errors.push("runtime_catalog_loader_hash_mismatch");
  ...
  if (
    catalog.source_catalog?.domain_pack_id !== "skinharmony"
    || expectedTopology.branch_count !== SKINHARMONY_CORE_TOPOLOGY_BASELINE.branch_count
    || expectedTopology.subbranch_count !== SKINHARMONY_CORE_TOPOLOGY_BASELINE.subbranch_count
  ) errors.push("runtime_catalog_topology_outdated");
  if (JSON.stringify(catalogTopologyProjection) !== JSON.stringify(sourceTopologyProjection)) {
    errors.push("runtime_source_catalog_topology_mismatch");
  }
  if (
  ...
  if (
    ...
    || validation.validated_branch_count !== expectedTopology.branch_count
    || validation.validated_subbranch_count !== expectedTopology.subbranch_count
    || validation.validated_node_count !== expectedTopology.node_count
  ...
  if (
    ...
    || supervisor.approved_node_count !== expectedTopology.node_count
    || supervisor.rejected_node_count !== 0
  ) errors.push("runtime_supervisor_attestation_invalid");
  if (
    manifest?.topology?.branch_count !== expectedTopology.branch_count
    || manifest?.topology?.subbranch_count !== expectedTopology.subbranch_count
    || manifest?.topology?.node_count !== expectedTopology.node_count
    || !Array.isArray(summaries)
    || summaries.length !== expectedTopology.node_count
    || new Set(summaries.map((node) => node.id)).size !== expectedTopology.node_count
  ...
  if (
    ...
    || expectedSubbranches.size !== expectedTopology.subbranch_count
    || descriptors.length !== expectedTopology.subbranch_count
    || actualSubbranches.size !== expectedTopology.subbranch_count
  ...
  if (registry.function_count !== expectedTopology.node_count) {
    errors.push("runtime_function_count_invalid");
  }
  const integrity = verifyRuntimeManifestArtifacts(manifest, manifestPath, {
    verifyContent: verifyArtifacts,
  });
  errors.push(...integrity.errors);
  if (integrity.unchecked_shards !== 0) errors.push("runtime_shards_unchecked");
  if (
    verifyArtifacts
    && integrity.content_verified_shards !== descriptors.length
  ) errors.push("runtime_shard_content_verification_incomplete");
  return {
    ok: errors.length === 0,
    errors,
    warnings: [],
...
function loadRuntimeManifest({
  ...
  const stat = fs.statSync(absolutePath);
  const cacheKey = `${absolutePath}:${stat.mtimeMs}:${stat.size}:${verifyArtifacts ? "full" : "lazy"}`;
...
function loadCatalog({
  catalogPath = process.env.NYRA_DEEP_BRANCH_V2_CATALOG_PATH
    || DEFAULT_CATALOG_PATH,
  manifestPath = process.env.NYRA_DEEP_BRANCH_V2_RUNTIME_MANIFEST_PATH
    || DEFAULT_RUNTIME_MANIFEST_PATH,
  forceReload = false,
  runtimeMode = process.env.NYRA_DEEP_BRANCH_V2_RUNTIME_MODE || "auto",
  verifyArtifacts = false,
} = {}) {
  if (!["auto", "lazy", "legacy"].includes(runtimeMode)) {
    return {
      ok: false,
      state: "runtime_mode_rejected",
      catalog: null,
      validation: {
        ok: false,
        errors: ["runtime_mode_invalid"],
        warnings: [],
        metrics: {},
      },
      runtime_lazy: runtimeMode !== "legacy",
    };
  }
  ...
  return useRuntimeManifest
    ? loadRuntimeManifest({ manifestPath, forceReload, verifyArtifacts })
