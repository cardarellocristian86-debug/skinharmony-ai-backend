...
test("validation and a lazy evaluated deep route stay below 256 MiB without reading the monolith", (t) => {
  const childScript = String.raw`
    ...
    const nodesPerSubbranch = ${nodesPerSubbranch};
    const env = {
      NYRA_DEEP_BRANCH_V2_ENABLED: "true",
      NYRA_DEEP_BRANCH_V2_MODE: "shadow",
      NYRA_DEEP_BRANCH_V2_BRANCHES: branchIds.join(","),
    ...
    const expectedBranchCount = branchIds.length;
    const expectedShardCount = loaded.manifest.topology.subbranch_count;
    const expectedNodeCount = expectedShardCount * nodesPerSubbranch;
    assert.equal(loaded.manifest.topology.branch_count, expectedBranchCount);
    ...
    assert.equal(routed.selected_branches.length, expectedBranchCount);
    ...
    assert.equal(routedNodeCount, expectedNodeCount);
    ...
    const budgets = {
      ...
      required_shard_count: expectedShardCount,
    ...
    const checks = {
      ...
      topology: routedNodeCount === budgets.required_node_count && routed.evaluations.length === 6,
  ...
  assert.equal(result.deep_route.routed_nodes, result.budgets.required_node_count);
...
test("missing, tampered, swapped, oversized and stale shards all fail closed", () => {
  ...
  const rejectedManifest = (label, { verifyArtifacts = false } = {}) => {
  ...
  try {
    ...
    const lazyTampered = loadCatalog({
      manifestPath,
      runtimeMode: "lazy",
      forceReload: true,
    });
    assert.equal(lazyTampered.ok, true, lazyTampered.validation.errors.join("\n"));
    const tamperedShard = loadRuntimeShard({
      loaded: lazyTampered,
      tenantId: "codexai",
      branchId: firstDescriptor.branch_id,
      subbranchId: firstDescriptor.subbranch_id,
      env: enabledEnv(),
    });
    assert.equal(tamperedShard.ok, false);
    assert(tamperedShard.errors.some((error) => error.startsWith(
      `runtime_shard_compressed_hash_mismatch:${firstDescriptor.branch_id}.${firstDescriptor.subbranch_id}`
    )));
    assert(
      rejectedManifest("tampered shard", { verifyArtifacts: true }).some((error) => error.startsWith(
    ...
    const swappedErrors = rejectedManifest("swapped shard", { verifyArtifacts: true });
    assert(
      swappedErrors.some((error) => (
        error.startsWith("compressed_shard_")
        && error.endsWith(`${firstDescriptor.branch_id}.${firstDescriptor.subbranch_id}`)
      ))
    );
    ...
    assert(
      rejectedManifest("oversized gzip shard", { verifyArtifacts: true }).some(
...
test("runtime packaging retains exactly the manifest generation and declared shards", () => {
  ...
  assert.equal(manifest.topology.branch_count, branchIds.length);
  assert.equal(manifest.topology.node_count, expectedShardCount * nodesPerSubbranch);
  assert.equal(shardFiles.length, expectedShardCount);
  assert.equal(manifest.shards.length, expectedShardCount);
});

test("generation cleanup removes stale current-fingerprint generations and preserves prior fingerprints", async () => {
  ...
  const { nyraBranchCatalog } = await import(pathToFileURL(coreModulePath).href);
  const expectedShardCount = nyraBranchCatalog("skinharmony").branches.reduce(
  ...
  const previousCatalogFingerprint = "d".repeat(64);
  const previousCatalogBinding = "e".repeat(64);
  ...
  const previousCatalogRoot = path.join(
    shardRoot,
    "v1",
    previousCatalogFingerprint,
    previousCatalogBinding
  );
  ...
  try {
    ...
    fs.mkdirSync(previousCatalogRoot, { recursive: true });
    ...
    fs.writeFileSync(path.join(previousCatalogRoot, "immutable.json.gz"), "preserve");
    ...
    assert.equal(cleanup.shard_file_count, expectedShardCount);
    ...
    assert.equal(fs.existsSync(previousCatalogRoot), true);
    assert.equal(
      fs.readFileSync(path.join(previousCatalogRoot, "immutable.json.gz"), "utf8"),
      "preserve"
    );
