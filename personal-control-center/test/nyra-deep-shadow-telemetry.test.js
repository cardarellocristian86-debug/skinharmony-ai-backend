      ...
      evaluations: [
        {
          ...
          evidence: [{ customer_name: "synthetic_customer" }],
        },
        {
...
test("shadow event retains only bounded routing telemetry and no prompt, evidence, request or node input", () => {
  const event = eventAt("2026-07-24T10:00:00.000Z", {
    localInterpretation: {
      ...
      raw_prompt: "synthetic_sensitive_prompt",
    },
    corePayload: {
      ...corePayload,
      request_id: "synthetic_request_reference",
      evidence: [{ customer_name: "synthetic_customer" }],
    },
  });
  ...
  for (const forbidden of [
    "synthetic_sensitive_prompt",
    "synthetic_customer",
    "synthetic_request_reference",
  ]) {
...
test("real catalog aggregation preserves every Core branch-qualified subbranch", async () => {
  const { nyraBranchCatalog } = await import(
    "../../services/universal-core-service/src/nyraBranchNetwork.js"
  );
  const coreCatalog = nyraBranchCatalog("skinharmony");
  const expectedBranchIds = coreCatalog.branches.map((branch) => branch.id);
  const expectedSubbranchIds = coreCatalog.branches.flatMap((branch) =>
    branch.subbranches.map((subbranchId) => `${branch.id}.${subbranchId}`)
  );
  ...
  assert(knownBranchIds.every((branchId) => expectedBranchIds.includes(branchId)));
  assert(knownSubbranchIds.every((subbranchId) => expectedSubbranchIds.includes(subbranchId)));
  assert.equal(new Set(knownSubbranchIds).size, knownSubbranchIds.length);
  assert.equal(aggregate.subbranch_usage.known_count, knownSubbranchIds.length);
  assert.equal(aggregate.subbranch_usage.unused_count, knownSubbranchIds.length);
});
