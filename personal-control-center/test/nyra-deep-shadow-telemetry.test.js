...
test("real catalog aggregation preserves every Core branch-qualified subbranch", async () => {
  ...
  assert.deepEqual(knownBranchIds, expectedBranchIds);
  assert.deepEqual(knownSubbranchIds, expectedSubbranchIds);
  assert.equal(new Set(knownSubbranchIds).size, expectedSubbranchIds.length);
  assert.equal(aggregate.subbranch_usage.known_count, expectedSubbranchIds.length);
  assert.equal(aggregate.subbranch_usage.unused_count, expectedSubbranchIds.length);
});
