...
function operationalFixture({
  ...
  const nodeContexts = lineage.map((lineageNode, index) => {
    const observedAtValue = 1_780_000_000_000;
    observedAt.push(observedAtValue);
    const opaque = encodeOpaqueNodeContext({
      node_id: lineageNode.node_id,
      capability_input: {
        node_id: lineageNode.node_id,
        synthetic_fixture: true,
      },
      evidence: [{
        evidence_id: `synthetic-evidence-${index}`,
        observed_at: observedAtValue,
      }],
      evidence_manifest: {
        fixture: "synthetic_operational_context",
        observed_at: observedAtValue,
      },
      policy_decisions: [{
        decision: "ALLOW",
    ...
    return {
      schema_version: OPAQUE_NODE_CONTEXT_SCHEMA_VERSION,
      node_id: lineageNode.node_id,
...
test("federation preserves an Ed25519-attested exact six-node lineage without leaking opaque Core context", () => {
  ...
  assert.equal(result.state, "operational_advisory_fallback_v1_authoritative");
  assert.equal(result.execution_authorized, false);
  assert.equal(result.core_final_authority, true);
  assert.deepEqual(result.selected_branches, []);
  assert.equal(result.evaluation.schema_version, EVALUATION_RESPONSE_SCHEMA_VERSION);
  assert.equal(result.evaluation.state, "operational_advisory_fallback");
  assert.equal(result.evaluation.evaluated_node_count, 6);
  assert.equal(result.evaluation.all_nodes_verified, false);
  assert.deepEqual(result.evaluation.lineage.nodes.map((node) => node.node_id), lineage.map((node) => node.node_id));
  assert(result.evaluation.lineage.nodes.some((node) => node.state !== "advisory_verified"));
