...
const expectedBranchCounts = Object.freeze({
  ...
  relational_supervision: 20,
  agent_orchestration: 20,
  ai_orchestration: 20,
  execution_planning: 10,
  parallel_coordination: 15,
  tenant_work_coordination: 19,
  agent_change_interlock: 8,
...
const expectedSubbranchCount = Object.values(expectedBranchCounts)
  .reduce((sum, count) => sum + count, 0);
const expectedNodeCount = expectedSubbranchCount * (2 + expectedLevel4Types.length);
const expectedFixtureGroups = Object.freeze([
  ["positive_tests", "ALLOW_ADVISORY"],
  ["negative_tests", "ABSTAIN"],
  ["adversarial_tests", "DENY"],
  ["regression_tests", "ALLOW_ADVISORY"],
]);
const expectedFixtureCount = expectedNodeCount * expectedFixtureGroups.length;
const catalogPath = path.resolve(
  process.env.NYRA_DEEP_V2_TEST_CATALOG_PATH
    || process.env.NYRA_DEEP_BRANCH_V2_CATALOG_PATH
    || DEFAULT_CATALOG_PATH
);
...
test("approved runtime catalog matches the authenticated live topology", () => {
  ...
  assert.deepEqual(topologyMetrics(loaded.catalog), {
    ...
    subbranch_count: expectedSubbranchCount,
    node_count: expectedNodeCount,
    level_counts: {
      1: expectedSubbranchCount,
      2: expectedSubbranchCount,
      3: expectedSubbranchCount,
      4: expectedSubbranchCount * expectedLevel4Types.length,
    },
    level4_type_counts: Object.fromEntries(
      expectedLevel4Types.map((type) => [type, expectedSubbranchCount])
    ),
  });
  assert.deepEqual(
  ...
  assert.equal(loaded.catalog.function_registry.functions.length, expectedNodeCount);
  assert.equal(
...
test("every L2-L4 node has an independent complete contract and exact parent topology", () => {
  ...
  for (const branch of data.branches) {
    for (const subbranch of branch.subbranches) {
      ...
      assert.deepEqual(
        level4.map((node) => node.node_type).sort(),
        [...expectedLevel4Types].sort()
      );
    }
  }
});

test("all embedded positive, negative, adversarial and regression cases are enforceable", () => {
  const data = catalog();
  ...
  assert.equal(fixtureBundle.fixture_count, expectedFixtureCount);
  const nodeIndex = new Map(data.nodes.map((node) => [node.id, node]));
  let executed = 0;
  for (const node of data.nodes) {
    for (const [field, expectedVerdict] of expectedFixtureGroups) {
  ...
  assert.equal(executed, expectedFixtureCount);
});
...
test("V1 rollback goldens execute against the real horizontal runtime and Core branch router", async () => {
  ...
  const packIds = ["generic", "suite", "smartdesk", "analyzer"];
  const catalogs = packIds.map((packId) => nyraBranchCatalog(packId));
  const unionById = new Map(catalogs.flatMap((item) => item.branches).map((item) => [item.id, item]));
  const catalogUnion = {
    ...catalogs[0],
    domain_pack_id: catalogGolden.input.domain_pack_id,
    branches: catalogGolden.output.branches.map((item) => unionById.get(item.id)),
  };
  assert.deepEqual(catalogUnion, catalogGolden.output);
  assert.equal(sha256(catalogUnion), catalogGolden.output_hash);
  assert.deepEqual(
    catalogs.map((item) => item.branches.filter((branch) => branch.work_phase === "domain").map((branch) => branch.id)),
    [[], ["suite_domain"], ["smartdesk_domain"], ["analyzer_domain"]],
  );
  const packForBranch = {
    suite_domain: "suite",
  ...
  for (const branchId of Object.keys(expectedBranchCounts)) {
    ...
    const actual = routeNyraBranches({
      ...golden.input,
      domainPackId: packForBranch[branchId] || "generic",
    });
    const normalized = { ...actual, domain_pack_id: golden.output.domain_pack_id };
    assert.deepEqual(normalized, golden.output, `V1 Core route changed for ${branchId}`);
    assert.equal(sha256(normalized), golden.output_hash);
...
test("all nodes fail closed under the independent Pass 4 semantic mutation matrix", () => {
  ...
  assert.equal(probes, expectedNodeCount * 26);
});
...
test("all nodes reject exact-envelope and coherently rebound Pass 6 mutations", () => {
  ...
  assert.equal(probes, expectedNodeCount * 11);
});
...
test("function registry rejects donor and malformed semantic observations for every node", () => {
  ...
  assert.equal(probes, expectedNodeCount * 5);
});
