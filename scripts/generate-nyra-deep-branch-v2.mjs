...
const DEEP_V2_EXCLUDED_BRANCHES = Object.freeze({
  tenant_work_coordination:
    "Tenant Work Gallery is governed by the V1/Core continuity runtime and has no reviewed Deep V2 research contracts in this release.",
});
...
function main() {
  ...
  const capturedAtInput = requiredArg(args, "captured-at");
  ...
  const sourceBranchIds = new Set(sourceCatalog.branches.map((branch) => branch.id));
  const missingExcludedBranches = Object.keys(DEEP_V2_EXCLUDED_BRANCHES)
    .filter((branchId) => !sourceBranchIds.has(branchId));
  if (missingExcludedBranches.length) {
    throw new Error(
      `Deep V2 exclusion references missing live branches: ${missingExcludedBranches.join(",")}`,
    );
  }
  const deepV2Branches = sourceCatalog.branches.filter(
    (branch) => !Object.hasOwn(DEEP_V2_EXCLUDED_BRANCHES, branch.id),
  );
  const liveBranchIds = new Set(deepV2Branches.map((branch) => branch.id));
  ...
  const expectedSubbranches = deepV2Branches.reduce((sum, branch) => sum + branch.subbranches.length, 0);
  ...
  const liveKeys = deepV2Branches.flatMap((branch) => branch.subbranches.map((subbranch) => `${branch.id}.${subbranch}`));
  ...
  const runtimeSha256 = sha256(
    fs.readFileSync(NYRA_DEEP_V2_RUNTIME_PATH, "utf8"),
  );
  ...
  const nodes = [];
  const branches = deepV2Branches.map((branch) => {
  ...
  const catalog = {
    ...
    source_catalog: {
      ...
      source_pin: V1_SOURCE_PIN,
      deep_v2_scope: {
        inclusion: "reviewed_research_contracts_only",
        excluded_branches: Object.entries(DEEP_V2_EXCLUDED_BRANCHES).map(
          ([id, reason]) => ({ id, reason }),
        ),
      },
      branches: deepV2Branches.map((branch) => ({
        id: branch.id,
        label: branch.label,
        work_phase: branch.work_phase,
        core_branch_bindings: branch.core_branch_bindings || [],
