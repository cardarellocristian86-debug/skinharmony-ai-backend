...
function main() {
  ...
  const candidateArtifact = readJsonArtifact(candidatePath, "Candidate");
  const fixtureArtifact = readJsonArtifact(fixturePath, "Fixture");
  const supervisorArtifact = readJsonArtifact(supervisorPath, "Supervisor");
  const candidate = candidateArtifact.value;
  const fixtures = fixtureArtifact.value;
  const supervisor = supervisorArtifact.value;
  const supervisorCandidateSha256 = requireSha256(
    supervisor?.candidate?.sha256,
    "supervisor.candidate.sha256"
  );
  const supervisorFixtureSha256 = requireSha256(
    supervisor?.candidate?.fixture_sha256,
    "supervisor.candidate.fixture_sha256"
  );
  if (candidateArtifact.byte_sha256 !== supervisorCandidateSha256) {
    throw new Error("Candidate byte SHA-256 does not match Supervisor binding");
  }
  if (fixtureArtifact.byte_sha256 !== supervisorFixtureSha256) {
    throw new Error("Fixture byte SHA-256 does not match Supervisor binding");
  }
  for (const [field, expected] of [
    ["byte_sha256_before", candidateArtifact.byte_sha256],
    ["byte_sha256_after", candidateArtifact.byte_sha256],
    ["fixture_byte_sha256_before", fixtureArtifact.byte_sha256],
    ["fixture_byte_sha256_after", fixtureArtifact.byte_sha256],
  ]) {
    if (
      supervisor.candidate?.[field] !== undefined
      && supervisor.candidate[field] !== expected
    ) {
      throw new Error(`Supervisor candidate.${field} does not match input bytes`);
    }
  }
  if (
    (supervisor.candidate?.byte_sha256_stable !== undefined
      && supervisor.candidate.byte_sha256_stable !== true)
    || (supervisor.candidate?.fixture_byte_sha256_stable !== undefined
      && supervisor.candidate.fixture_byte_sha256_stable !== true)
  ) {
    throw new Error("Supervisor did not attest stable candidate and fixture bytes");
  }
  ...
  if (
    ...
    || supervisorSummary?.rejected_nodes !== 0
    || supervisorSummary?.deploy_allowed !== false
  ) {
    throw new Error("Supervisor summary does not authorize catalog admission");
  }
  const approvedNodeIds = Array.isArray(supervisor.approved_node_ids)
    ? [...supervisor.approved_node_ids].sort()
    : [];
  const candidateNodeIds = candidate.nodes.map((node) => node.id).sort();
  if (
    approvedNodeIds.length !== candidateNodeIds.length
    || JSON.stringify(approvedNodeIds) !== JSON.stringify(candidateNodeIds)
    || !Array.isArray(supervisor.rejected_node_ids)
    || supervisor.rejected_node_ids.length !== 0
  ) {
    throw new Error("Supervisor approved/rejected node id sets do not match candidate");
  }
  if (
    supervisor.candidate?.runtime_sha256 !== candidate.runtime_sha256
    || supervisor.candidate?.generator_sha256 !== candidate.generator_sha256
    || supervisor.candidate?.registry_hash !== candidate.function_registry.registry_hash
  ) {
    throw new Error("Supervisor implementation/catalog bindings do not match candidate");
  }
  const supervisorBinding = supervisorAttestationBinding(supervisor);
  const supervisorBindingSha256 = supervisorAttestationHash(supervisor);
  ...
  promoted.supervisor_admission = {
    ...
    candidate_byte_sha256: candidateArtifact.byte_sha256,
    fixture_byte_sha256: fixtureArtifact.byte_sha256,
  ...
  process.stdout.write(`${JSON.stringify({
    ...
    supervisor_binding_schema_version: supervisorBinding.schema_version,
    supervisor_binding_sha256: supervisorBindingSha256,
