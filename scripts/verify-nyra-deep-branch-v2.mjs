...
const CORE_SKINHARMONY_TOPOLOGY = Object.freeze({
  projection: branchTopologyProjection(
    CORE_SKINHARMONY_CATALOG.branches.filter(
      (branch) => !Object.hasOwn(DEEP_V2_EXCLUDED_BRANCHES, branch.id),
    ),
  ),
  ...catalogTopology({
    branches: CORE_SKINHARMONY_CATALOG.branches.filter(
      (branch) => !Object.hasOwn(DEEP_V2_EXCLUDED_BRANCHES, branch.id),
    ),
  }),
});
...
function main() {
  ...
  const coreBranchIds = new Set(
    CORE_SKINHARMONY_CATALOG.branches.map((branch) => branch.id),
  );
  const missingExcludedBranches = Object.keys(DEEP_V2_EXCLUDED_BRANCHES)
    .filter((branchId) => !coreBranchIds.has(branchId));
  if (missingExcludedBranches.length) {
    errors.push(
      `deep_v2_exclusion_missing_from_core:${missingExcludedBranches.join(",")}`,
    );
  }
  if (!deepEqualJson(
    catalog.source_catalog?.deep_v2_scope,
    {
    ...
    errors.push("deep_v2_scope_mismatch");
  }
  if (!deepEqualJson(
