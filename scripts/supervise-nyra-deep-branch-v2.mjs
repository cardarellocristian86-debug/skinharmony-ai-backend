...
function globalPreflight({
  ...
  return {
    ...
    deep_v2_scope:
      candidate.source_catalog?.deep_v2_scope?.inclusion
        === "reviewed_research_contracts_only"
      && deepEqualJson(
        candidate.source_catalog?.deep_v2_scope?.excluded_branches,
        Object.entries(DEEP_V2_EXCLUDED_BRANCHES).map(
          ([id, reason]) => ({ id, reason }),
        ),
      ),
...
function main() {
  ...
  try {
    ...
    const coreBranchIds = new Set(coreCatalog.branches.map((branch) => branch.id));
