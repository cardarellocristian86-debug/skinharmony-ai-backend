...
function exactNodeCoverage(catalog) {
  ...
  const coreBranchIds = new Set(coreTopology.branches.map((branch) => branch.id));
  ...
  if (missingExcludedBranches.length) {
    throw new Error(
      `Deep V2 exclusion is absent from Core: ${missingExcludedBranches.join(",")}`,
    );
  }
  const expectedScope = {
  ...
  if (
    JSON.stringify(catalog.source_catalog?.deep_v2_scope)
      !== JSON.stringify(expectedScope)
  ) {
    throw new Error("Deep V2 scope does not match the reviewed exclusion policy");
  }
