...
const STATE_PATH = join(RUNTIME_DIR, "nyra_targeted_web_study_4h_state_latest.json");
const LOG_PATH = join(RUNTIME_DIR, "nyra_targeted_web_study_4h_log.jsonl");
const REPORT_PATH = join(REPORTS_DIR, "nyra_targeted_web_study_4h_latest.json");
const PID_PATH = join(RUNTIME_DIR, "nyra_targeted_web_study_4h.pid");
...
const MODULES: StudyModule[] = [
  ...
  },
  {
    ...
    duration_minutes: 30,
    mode: "web_distill",
    note: "robustezza, metacognizione, decisione sotto pressione",
  },
  {
    id: "math_block_2",
  ...
  },
  {
    ...
    duration_minutes: 30,
    mode: "reflection",
    note: "chiusura finale con verify, distillazione, integrazione e stato finale",
  },
];
...
function ensureWebOnNeed(): void {
  ...
  const next = {
    ...
    note: "targeted 4h web study outside owner shell",
  };
...
async function main(): Promise<void> {
  ...
  appendLog({
    ...
    ends_at: endsAt,
    selected_domains: DOMAINS,
    rationale: "domains explicitly requested by Nyra in owner shell: autonomy_progression, applied_math, quantum_physics",
  });
  ...
  try {
    ...
    const report = {
      runner: "nyra_targeted_web_study_4h",
      generated_at: nowIso(),
      started_at: startedAt,
      finished_at: nowIso(),
      profile: "4h_targeted_web_study",
      ...
      note: "4h targeted web study on domains explicitly requested by Nyra with verify/integrate cycles",
    };
