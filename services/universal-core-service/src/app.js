...
import { resolveAgenticHardBudgetStop } from "./agenticRuntimeFlags.js";
import {
...
export function createUniversalCoreService(options = {}) {
  ...
  const agenticBudgetMode = ["off", "observe", "soft_enforce"].includes(requestedAgenticBudgetMode)
    ...
    : "observe";
  const agenticHardBudgetStopStatus = resolveAgenticHardBudgetStop(
    options.agenticHardBudgetStop ?? process.env.AGENTIC_HARD_BUDGET_STOP,
  );
  ...
  if (aiLearningFactoryMode !== "off") {
    mountAiLearningFactoryRoutes({
      app,
      readAuth: coreAuth(SCOPES.READ_DECISION),
      governAuth: coreAuth(SCOPES.WRITE_DECISION),
      telemetryStore: aiLearningTelemetryStore,
  ...
  if (agenticEfficiencyMode !== "off") {
    mountAgenticEfficiencyRoutes({
      ...
      hardBudgetStopStatus: agenticHardBudgetStopStatus,
  ...
  app.post("/v1/generic-agents/runs", coreAuth(SCOPES.WRITE_DECISION), async (req, res) => {
  ...
  app.post("/v1/generic-agents/orchestration/:planId/join", coreAuth(SCOPES.WRITE_DECISION), async (req, res) => {
    try {
  ...
  app.get("/healthz", async (req, res) => {
    ...
    res.status(renderReady ? 200 : 503).json({
      ...
      agentic_efficiency: {
        ...
        hard_budget_stop: agenticHardBudgetStopStatus.active,
        hard_budget_stop_state: agenticHardBudgetStopStatus.state,
        hard_budget_stop_advisory_only: agenticHardBudgetStopStatus.advisory_only,
        hard_budget_stop_reason: agenticHardBudgetStopStatus.reason,
        execution_enabled: false,
