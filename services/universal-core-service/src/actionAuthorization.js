...
function hasValidCoreAdminBootstrapContext(body = {}) {
  const memory = body.memory_context;
  return memory && typeof memory === "object" && !Array.isArray(memory) &&
    String(memory.schema_version || "") === "tenant_memory_context_v1" &&
    String(memory.tenant_id || "") === "codexai" &&
    Number.isInteger(memory.revision) && memory.revision >= 0 &&
    /^[a-zA-Z0-9][a-zA-Z0-9_-]{1,63}$/.test(String(body.agent_id || "")) &&
...
export function buildActionAuthorization(decisionContract = {}, body = {}) {
  ...
  const sandboxedScopedWork =
    ...
    body.cross_tenant !== true &&
    body.configuration_changes !== true;
  const authenticatedTenant = String(body.authenticated_tenant_id || "");
  const requestedTenant = String(body.tenant_id || "");
  const policyRegistrySnapshotMutation =
    String(body.operation_class || "") === "policy_registry_snapshot_mutation" &&
    new Set(["policy.snapshot.activate", "policy.snapshot.rollback", "policy.snapshot.reconcile"])
      .has(String(body.action_type || "").toLowerCase()) &&
    ownerConfirmed && body.request_bound_owner_confirmation === true &&
    body.owner_context_verified === true && authenticatedTenant.length > 0 &&
    requestedTenant === authenticatedTenant && body.work_preflight_ready === true &&
    body.external_side_effect === false && body.provider_execution === false &&
    body.contains_secret === false && body.secret_value_transmitted === false &&
    body.cross_tenant === false && body.bypass_orchestrator === false &&
    body.destructive === false && body.rollback_ready === true && body.audit_ready === true;
  const ownerSovereigntyOperation = [
    "owner_confirmed_governed_action",
    "owner_confirmed_high_impact",
    "owner_confirmed_unverified_learning_change",
    "owner_root_governance_amendment",
    "core_policy_safety_bugfix",
    "production_feature_flag_and_authority_change",
    "nyra_v2_authority_promotion",
    "dtt_live_feature_activation",
    "production_deploy",
    "draft_code_change",
  ].includes(String(body.operation_class || ""));
  // The endpoint derives these identity and binding fields server-side after
  // validating a short-lived HMAC owner assertion for this exact request.
  // This is the governed escape from the historical closed operation allowlist;
  // it does not override absolute safety blocks.
  const ownerConfirmedGovernedAction =
    ownerSovereigntyOperation &&
    ownerConfirmed &&
    ...
    body.secret_value_transmitted !== true &&
    body.bypass_orchestrator === false &&
    body.legal_violation !== true &&
    body.target_authority_verified !== false &&
    body.actor_authorized_for_target !== false &&
    body.audit_ready === true &&
    (body.external_side_effect !== true || body.rollback_ready === true) &&
  ...
  const adminControlRoomEnvironmentMentioned = [
    ...(Array.isArray(body.allowed_environment_variables) ? body.allowed_environment_variables : []),
    ...(Array.isArray(body.confirmation_environment_variables) ? body.confirmation_environment_variables : []),
  ].some((key) => String(key).startsWith("CORE_ADMIN_"));
  const adminControlRoomValueFieldPresent = Object.keys(body || {})
    .some((key) => String(key).startsWith("CORE_ADMIN_"));
  const adminControlRoomSecretAttempt =
    ...
    adminControlRoomEnvironmentMentioned ||
    adminControlRoomValueFieldPresent;
  ...
  const adminControlRoomSecretConfiguration =
    ...
    hasValidCoreAdminBootstrapContext(body) &&
    body.external_side_effect === true && body.contains_customer_data === false && body.contains_secret === false &&
  ...
  return {
    ...
    scope: tenantScopedRead
      ...
      : sandboxedScopedWork
        ...
        : policyRegistrySnapshotMutation
          ? "policy_registry_snapshot_mutation"
