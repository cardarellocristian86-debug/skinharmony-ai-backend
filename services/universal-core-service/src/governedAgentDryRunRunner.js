...
export function createGovernedAgentDryRunRunner({ queueStore, audit = null, now = () => new Date(), maxJobsPerTick = 2 } = {}) {
  ...
  return {
    tick({ tenant_id }) {
      const expiry = queueStore.expire({ tenant_id });
      const completed = [];
      for (let index = 0; index < limit; index += 1) {
        const job = queueStore.claim({ tenant_id });
        if (!job) break;
        ...
        completed.push(queueStore.complete({ tenant_id, job_id: job.job_id, result }));
        audit?.append?.("governed_agent_queue_dry_run_completed", { tenant_id, job_id: job.job_id, worker_id: job.worker_id });
      }
      return { execution_mode: "dry_run", expired: expiry.expired, completed, metrics: queueStore.metrics({ tenant_id }) };
    },
  };
}
