...
function branchAllowedForEntitlement(branchId, entitlementDomainPackId) {
  const requiredPack = NYRA_DEEP_V2_DOMAIN_BRANCH_PACK[String(branchId || "")];
  return !requiredPack
    || entitlementDomainPackId === NYRA_DEEP_V2_CATALOG_SCOPE
    || requiredPack === entitlementDomainPackId;
}
...
function corePolicyHash({ selectedByCore, openedBranchIds, workPreflight, entitlementDomainPackId }) {
  return sha256({
    catalog_scope: NYRA_DEEP_V2_CATALOG_SCOPE,
    entitlement_domain_pack: entitlementDomainPackId,
    selected_by_core: {
...
function compactPreviewBranches(values) {
  ...
  const output = [];
  for (const branch of values.slice(0, MAX_BRANCHES)) {
...
function previewResponseValid(data, context) {
  ...
  if (selected.length !== opened.size || [...opened].some((branchId) => !selected.includes(branchId))) {
    return { ok: false, reason: "nyra_deep_branch_v2_preview_incomplete" };
  }
...
export function createNyraDeepBranchV2Client({
  ...
  function envelopeContext({
    ...
    const entitlementPack = String(entitlementDomainPackId || "").trim();
    if (!NYRA_DEEP_V2_ENTITLEMENT_PACKS.has(entitlementPack)) {
      return { ok: false, response: fallback({ tenantId, requestId, state: "entitlement_denied_v1_authoritative", reason: "nyra_deep_branch_v2_entitlement_pack_invalid" }) };
    }
    const nowMs = now();
    ...
    if (openedBranchIds.some((openedBranchId) => !branchAllowedForEntitlement(openedBranchId, entitlementPack))) {
      return { ok: false, response: fallback({ tenantId, requestId, state: "entitlement_denied_v1_authoritative", reason: "nyra_deep_branch_v2_branch_outside_entitlement_pack" }) };
    }
    const policyHash = corePolicyHash({
      selectedByCore,
      openedBranchIds,
      workPreflight,
      entitlementDomainPackId: entitlementPack,
    });
    ...
    const envelope = {
      ...
      domain_pack: NYRA_DEEP_V2_CATALOG_SCOPE,
      catalog_scope: NYRA_DEEP_V2_CATALOG_SCOPE,
      entitlement_domain_pack: entitlementPack,
      opened_branch_ids: openedBranchIds,
