...
export function createNyraDeepBranchV2Attester({
  ...
  function requirementBindings({ branchId, subbranchId } = {}) {
    ...
    const bindings = loaded.nodes.flatMap((node) => node.required_evidence.map((requirement, index) => {
      const acceptance = requirement?.acceptance_program || {};
      return {
        ...
        evidence_type: String(requirement.evidence_type || ""),
        semantic_claim_hash: String(requirement.semantic_claim_hash || ""),
        required_claim: String(acceptance.required_claim || requirement.description || ""),
        required_content_tag: String(acceptance.required_content_tag || requirement.content_tag || ""),
    ...
    if (bindings.some((binding) => (
      ...
      return {
        ok: false,
        reason: "nyra_deep_v2_evidence_acceptance_contract_invalid",
  ...
  function prepare({
    ...
    const entitlementDomain = safeString(entitlementDomainPackId, 128);
    if (!tenant || !request || !domain) return { ok: false, reason: "nyra_deep_v2_tenant_request_domain_required" };
    if (domain !== NYRA_DEEP_V2_CATALOG_SCOPE) return { ok: false, reason: "nyra_deep_v2_domain_pack_not_supported" };
    if (!NYRA_DEEP_V2_ENTITLEMENT_PACKS.has(entitlementDomain)
      || !branchAllowedForEntitlement(branchId, entitlementDomain)) {
      return { ok: false, reason: "nyra_deep_v2_entitlement_pack_not_supported" };
    }
    ...
    const envelope = {
      ...
      catalog_scope: domain,
      entitlement_domain_pack: entitlementDomain,
      branch_ref: ledger.reference("branch", loaded.branchId),
      subbranch_ref: ledger.reference("subbranch", { branch_id: loaded.branchId, subbranch_id: loaded.subbranchId }),
      catalog: clone(loaded.artifact),
      core_context: {
  ...
  function prepareOperational({
    ...
    if (
      !tenant || !request || !preflight || !branch || !subbranch || domain !== NYRA_DEEP_V2_CATALOG_SCOPE
      || !NYRA_DEEP_V2_ENTITLEMENT_PACKS.has(entitlementDomain)
      || !branchAllowedForEntitlement(branch, entitlementDomain)
