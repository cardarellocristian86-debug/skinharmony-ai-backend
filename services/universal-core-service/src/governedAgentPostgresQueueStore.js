...
export function createGovernedAgentPostgresQueueStore({ connectionString, pool = null, now = () => new Date() } = {}) {
  ...
  return {
    ...
    async complete({ tenant_id, job_id, result = {} }) { const tenantId=text(tenant_id,"tenant_id",120),jobId=text(job_id,"job_id",160); await init(); const current=await db.query("SELECT agent_id,plan_id FROM governed_agent_queue_jobs WHERE tenant_id=$1 AND job_id=$2 AND status='claimed'",[tenantId,jobId]); if(!current.rows[0])throw new Error("queue_job_not_claimed");const guarded=guardInterAgentEnvelope({tenant_id:tenantId,from_agent_id:current.rows[0].agent_id,to_agent_id:"universal-core",thread_id:current.rows[0].plan_id,body:result});const stored=guarded.allowed?(guarded.value&&typeof guarded.value==="object"?guarded.value:{}):{schema_version:"inter_agent_untrusted_envelope_v1",state:"quarantined",propagation_allowed:false,quarantine:guarded.quarantine}; const found=await rows("UPDATE governed_agent_queue_jobs SET status=$3,completed_at=NOW(),updated_at=NOW(),result=$4::jsonb WHERE tenant_id=$1 AND job_id=$2 AND status='claimed' RETURNING *",[tenantId,jobId,guarded.allowed?"completed":"quarantined",JSON.stringify(stored)]); if(!found[0]) throw new Error("queue_job_not_claimed"); return found[0]; },
