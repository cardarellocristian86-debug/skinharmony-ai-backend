...
export function createIcfKernel({ audit, storageRoot, mode = process.env.CORE_ICF_MODE || "shadow" } = {}) {
  ...
  return {
    ...
    globalJoin(tenantId, workId, snapshot, reality = {}) {
      const work = scoped(tenantId, workId); if (work.closure !== "CLOSING" || work.closure_snapshot?.state_digest !== snapshot?.state_digest) return { ok: false, error: "closure_snapshot_invalid" };
      const required = Array.isArray(work.covenant?.outcomes) ? work.covenant.outcomes : [];
      const forbidden = Array.isArray(work.covenant?.anti_goals) ? work.covenant.anti_goals : [];
      const observedText = JSON.stringify(reality); const missing = required.filter((item) => !observedText.includes(String(item))); const violated = forbidden.filter((item) => reality?.[item] === true || reality?.[item] === "true" || (reality?.violations || []).includes(item));
      const result = { join: "GLOBAL_INTENT_JOIN", verified: missing.length === 0 && violated.length === 0, snapshot_digest: snapshot.state_digest, missing_outcomes: missing, violated_anti_goals: violated };
