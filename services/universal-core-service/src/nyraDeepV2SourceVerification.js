...
const REGISTRY_SOURCE_TYPE_MAP = Object.freeze({
  official_documentation: "official",
  regulatory_source: "regulator",
  peer_reviewed: "academic",
  scientific_database: "academic",
  standards_body: "standards",
  vendor_primary: "manufacturer",
  authorized_dataset: "academic",
});
...
function normalizedRegistryDomain(value) {
  if (typeof value !== "string") return "";
  const host = value.trim().toLowerCase();
  if (!host || host.length > 253 || !host.includes(".") || net.isIP(host) !== 0) return "";
  return host.split(".").every((label) => (
    label.length > 0
    && label.length <= 63
    && /^[a-z0-9](?:[a-z0-9-]*[a-z0-9])?$/.test(label)
...
function normalizedSourceRegistry(sourceRegistry) {
  const invalid = () => {
    throw new TypeError("nyra_deep_v2_source_registry_invalid");
  };
  if (!sourceRegistry || typeof sourceRegistry !== "object" || Array.isArray(sourceRegistry)) invalid();
  const policy = sourceRegistry.branch_source_policy;
  if (!policy || typeof policy !== "object" || policy.fail_closed_on_unknown_source !== true) invalid();
  if (!Array.isArray(sourceRegistry.sources) || sourceRegistry.sources.length < 1) invalid();
  ...
  const sources = sourceRegistry.sources.map((source) => {
    ...
    const sourceType = REGISTRY_SOURCE_TYPE_MAP[String(source.source_type || "").trim()];
    const reliabilityScore = Number(source.reliability_score);
    if (
    ...
    if (!Array.isArray(source.allowed_domains) || !Array.isArray(source.allowed_operations)) invalid();
    sourceIds.add(sourceId);

    const allowedDomains = [];
    for (const value of source.allowed_domains) {
      const domain = normalizedRegistryDomain(value);
      if (!domain) invalid();
      if (!allowedDomains.includes(domain)) allowedDomains.push(domain);
    }
    const allowedOperations = [];
    for (const value of source.allowed_operations) {
      if (typeof value !== "string" || !value.trim()) invalid();
      const operation = value.trim();
      if (!allowedOperations.includes(operation)) allowedOperations.push(operation);
    }
    return Object.freeze({
      ...
      canFetchDocument: allowedOperations.includes("fetch_document"),
    });
  });
  const rawBranchBindings = sourceRegistry.branch_bindings;
  if (!rawBranchBindings || typeof rawBranchBindings !== "object" || Array.isArray(rawBranchBindings)) invalid();
  const branchBindings = new Map();
  for (const [branchId, values] of Object.entries(rawBranchBindings)) {
    if (!SOURCE_ID_PATTERN.test(branchId) || !Array.isArray(values) || values.length < 1) invalid();
    const bindings = [...new Set(values.map((value) => String(value || "").trim()))];
    if (bindings.length !== values.length || bindings.some((sourceId) => !sourceIds.has(sourceId))) invalid();
    branchBindings.set(branchId, Object.freeze(bindings));
  }
  if (branchBindings.size < 1) invalid();
  return Object.freeze({
...
function registrySourceFor(source, url, sourceRegistry, branchId) {
  const suppliedRegistrySourceId = source?.registry_source_id;
  const registrySourceId = suppliedRegistrySourceId === undefined || suppliedRegistrySourceId === null
    ? null
    : String(suppliedRegistrySourceId).trim();
  if (registrySourceId !== null && !SOURCE_ID_PATTERN.test(registrySourceId)) return null;
  const allowedSourceIds = sourceRegistry.branchBindings.get(branchId);
  if (!allowedSourceIds) return null;
  return sourceRegistry.sources.find((registrySource) => (
    registrySource.active
    && registrySource.canFetchDocument
    && allowedSourceIds.includes(registrySource.sourceId)
    && registrySource.allowedDomains.length > 0
    && (registrySourceId === null || registrySource.sourceId === registrySourceId)
    && registrySource.allowedDomains.some((domain) => (
...
function receiptPayload({
  ...
  return {
    ...
    source_id: sourceId,
    registry_source_id: registrySource.sourceId,
    source_type: registrySource.sourceType,
    reliability_score: registrySource.reliabilityScore,
...
export function createNyraDeepV2SourceVerifier({
  ...
  const trustedSources = normalizedSourceRegistry(sourceRegistry);
  ...
  async function verifyOne(source, branchId) {
    ...
    const registrySource = registrySourceFor(source, url, trustedSources, branchId);
    if (!registrySource) {
      return { ok: false, source_id: sourceId, reason: "nyra_deep_v2_source_registry_rejected" };
    }
  ...
  async function verifySources(sources, { branchId } = {}) {
    ...
    if (!SOURCE_ID_PATTERN.test(String(branchId || "")) || !trustedSources.branchBindings.has(branchId)) {
      return {
        ok: false,
        reason: "nyra_deep_v2_source_branch_binding_rejected",
        receipts: [],
        rejected: sources.map((source) => ({
          source_id: String(source?.id || "").trim() || null,
          reason: "nyra_deep_v2_source_branch_binding_rejected",
        })),
      };
    }
    const results = await concurrentMap(
      sources,
      MAX_CONCURRENCY,
      (source) => verifyOne(source, branchId),
    );
