...
const PERSISTENT_LEDGER_SCHEMA_VERSION = "nyra_deep_v2_evidence_file_state_v1";
...
export function createNyraDeepV2EvidenceLedger({
  ...
  function persistentStateSignature(unsigned) {
    return hmac(key, "persistent-ledger-state/v1", unsigned);
  }

  function decodePersistentState(state) {
    if (state === null) return { sequence: 0, records: new Map() };
    if (
      !exactKeys(state, ["schema_version", "sequence", "records", "signature"])
      || state.schema_version !== PERSISTENT_LEDGER_SCHEMA_VERSION
      || !Number.isSafeInteger(state.sequence)
      || state.sequence < 0
      || !Array.isArray(state.records)
      || state.records.length > capacity
      || !SHA256_PATTERN.test(String(state.signature || ""))
    ) throw new Error("nyra_deep_v2_ledger_state_corrupt");
    const unsigned = {
      schema_version: state.schema_version,
      sequence: state.sequence,
      records: state.records,
    };
    if (!safeEqual(state.signature, persistentStateSignature(unsigned))) {
      throw new Error("nyra_deep_v2_ledger_state_corrupt");
    }
    const decoded = new Map();
    for (const record of state.records) {
      if (!verifyRecord(record) || decoded.has(record.record_ref)) {
        throw new Error("nyra_deep_v2_ledger_state_corrupt");
      }
      decoded.set(record.record_ref, Object.freeze(record));
    }
    return { sequence: state.sequence, records: decoded };
  }

  function encodePersistentState(recordMap, stateSequence) {
    const unsigned = {
      schema_version: PERSISTENT_LEDGER_SCHEMA_VERSION,
    ...
    return {
      ...unsigned,
      signature: persistentStateSignature(unsigned),
    };
  }

  function replaceLocalState(state) {
    records.clear();
    for (const [recordRef, record] of state.records) records.set(recordRef, record);
    sequence = Math.max(sequence, state.sequence);
  }

  if (persistentState) {
    replaceLocalState(decodePersistentState(persistentState.read()));
  }

  function save(record) {
    const sealed = { ...record, signature: recordSignature(record) };
    if (persistentState) {
      const persisted = persistentState.update((current) => {
        const latest = decodePersistentState(current);
        const existing = latest.records.get(sealed.record_ref);
        if (existing && nyraDeepV2CanonicalJson(existing) !== nyraDeepV2CanonicalJson(sealed)) {
          throw new Error("nyra_deep_v2_ledger_record_conflict");
        }
        latest.records.set(sealed.record_ref, Object.freeze(sealed));
        while (latest.records.size > capacity) trimRecords(latest.records);
        latest.sequence = Math.max(latest.sequence, sequence);
        return {
          state: encodePersistentState(latest.records, latest.sequence),
          result: latest,
        };
      });
      replaceLocalState(persisted);
    } else {
