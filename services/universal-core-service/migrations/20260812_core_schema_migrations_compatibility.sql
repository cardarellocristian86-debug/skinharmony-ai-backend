...
-- These three fields belong only to causal rows.  Bootstrap rows predate
-- causal continuity and intentionally have no causal digest/state/checkpoint.
-- Dropping these constraints preserves all rows; causal inserts still supply
-- all three values and causal readback validates its own row exactly.
ALTER TABLE core_schema_migrations
