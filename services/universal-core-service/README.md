...
Il primo percorso live e volutamente piccolo e separato dalle code generiche: un owner OAuth puo confermare una sola esecuzione firmata e legata alla richiesta, che produce al massimo tre chiamate **Researcher → Architecture/Code Specialist → Nyra Supervisor**. Il task e limitato a 300 caratteri, ogni risposta a 200 token, non sono disponibili browser, tool, azioni esterne, retry delle chiamate modello o apprendimento. Task, contesto progetto e output degli stage precedenti sono dichiarati `UNTRUSTED DATA` in tutti e tre i prompt: non possono cambiare ruolo, policy o vincoli. Ogni esecuzione riceve un contesto progetto canonico, tenant-scoped e limitato in dimensione; solo `decisions` e `evidence` revisionati sono affidabili, mentre handoff e output precedenti marcati `UNREVIEWED` restano bozze. `POST /v1/generic-agents/providers/openai/multi-agent-runs` restituisce subito un `run_id`; il kill signal annulla la chiamata in corso e blocca gli stage successivi. Checkpoint e audit non contengono mai task, chiave provider o output del modello in chiaro: stato, timestamp e output redatti opzionali sono conservati dal primo checkpoint come envelope AES-256-GCM autenticato e legato a tenant, run e workflow. Dopo un restart anche `cancelled`, `failed` e un run trovato ancora `running` mantengono `created_at`, `started_at` e un `completed_at` durabile; la promozione `running → interrupted` emette anche un audit terminale senza testo. Le transizioni terminali ritentano il commit atomico e, se lo storage resta indisponibile, falliscono chiuse senza dichiarare o auditare un esito non salvato. Solo la route result con prova owner valida restituisce eventuale testo decifrato. L'envelope espone soltanto un `key_id` non segreto. La chiave dedicata corrente è `CORE_TENANT_OPENAI_OUTPUT_ENCRYPTION_SECRET`, generata e conservata da Render separatamente dai segreti di firma; durante una rotazione, le precedenti possono essere fornite temporaneamente e solo in lettura tramite `CORE_TENANT_OPENAI_OUTPUT_ENCRYPTION_SECRET_PREVIOUS` (lista separata da virgole), poi rimosse al termine della retention. Se resta disponibile soltanto una chiave precedente, Core puo decifrare un checkpoint attivo per mostrarlo come `interrupted` read-only, conservando i timestamp originali, ma non lo riscrive, non inventa `completed_at` e non emette un audit terminale finche non torna disponibile una chiave corrente. Senza una chiave corrente stabile di almeno 32 caratteri, `terminal_output_recovery_configured` e `execution_available` sono falsi e Core non avvia alcun run live; non esistono fallback casuali per la cifratura persistente. Il servizio MCP registra poi l'esito terminale nella cartella logica PostgreSQL `PROJECTS/<project_id>/`, aggiornando `STATE.md`, `HANDOFF.md` e `RUNS/<run_id>.md` senza promuovere automaticamente il testo del modello a decisione o prova. Una conferma firmata e monouso e registrata nel database del vault come hash, quindi non puo essere riutilizzata dopo un riavvio. `execution_enabled` del vault resta `false`: non esiste un interruttore globale; la sola disponibilita e il workflow fisso richiedono ogni volta tenant configurato e conferma owner.

La promozione controllata di decisioni ed evidenze usa un gate separato: `POST /v1/generic-agents/providers/openai/project-reviews/authorize`. Universal Core non riceve il testo della review, ma soltanto un envelope canonico con progetto, run, revisione attesa, disposizione, digest SHA-256, conteggi e chiave di idempotenza. La richiesta deve avere scope `write:decision`, una conferma OAuth owner firmata sul body esatto e una assertion monouso; replay, firma scaduta, tenant diverso o campi aggiuntivi falliscono chiusi. La risposta contiene solo `allowed`, un `authorization_id` e gli stessi campi di binding. Il gate resta disponibile anche quando il provider non e configurato o il runner non e attivo, cosi l'owner puo revisionare un run gia esistente senza riattivare OpenAI. Nessun contenuto viene salvato da questa route.
