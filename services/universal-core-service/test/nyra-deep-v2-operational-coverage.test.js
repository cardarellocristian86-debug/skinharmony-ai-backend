...
const EXPECTED_BRANCH_IDS = CORE_SKINHARMONY_CATALOG.branches.map((branch) => branch.id);
const EXPECTED_SUBBRANCH_COUNT = CORE_SKINHARMONY_CATALOG.branches.reduce(
...
test("all live V2 shards are Core-opened, attested, and operationally evaluated as an exact L2-to-L4 lineage", () => {
  ...
  assert.equal(loaded.manifest.shards.length, EXPECTED_SUBBRANCH_COUNT);
  ...
  assert.deepEqual(branches, EXPECTED_BRANCH_IDS);
