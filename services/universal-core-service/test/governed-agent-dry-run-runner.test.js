...
test("dry-run runner completes only dependency-ready jobs and reaps deadlines", () => {
  ...
  const outcome = runner.tick({ tenant_id: "runner-a" });
  ...
  assert.equal(runner.tick({ tenant_id: "runner-a" }).expired, 1);
  assert.equal(store.metrics({ tenant_id: "runner-a" }).status_counts.expired, 1);
});
