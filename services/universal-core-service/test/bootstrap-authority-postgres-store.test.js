...
test("migration is additive, immutable, and supports one un-attested local P1363 key", async () => { const state = poolWith(() => ({ rowCount: 0, rows: [] })); const store = createPostgresBootstrapAuthorityStore({ pool: state.pool }); const ready = await store.initialize(); const sql = state.calls[0].sql; const convergence = state.calls.find((entry) => String(entry.sql).includes("required_columns(table_name,column_name)")).sql; assert.match(sql, /local_pin/); assert.match(sql, /ECDSA_P256_SHA256_P1363/); assert.match(sql, /UNATTESTED_LOCAL_SOFTWARE/); assert.match(sql, /core_bootstrap_one_active_key_per_tenant_idx/); assert.match(sql, /WHERE status = 'ACTIVE'/); assert.match(convergence, /i\.indnkeyatts=1 AND i\.indnatts=1/); assert.match(convergence, /a\.attname='tenant_id'/); assert.match(convergence, /= 'status=''ACTIVE'''/); assert.equal(ready.automatic_trust_installation, false); assert.equal(state.calls.some((entry) => String(entry.sql).startsWith("INSERT INTO core_bootstrap_trust_keys")), false); assert.doesNotMatch(sql, /private_key/i); });
