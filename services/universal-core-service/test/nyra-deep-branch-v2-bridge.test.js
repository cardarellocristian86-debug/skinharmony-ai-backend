...
const MCP_REQUEST_SECRET = crypto.randomBytes(32).toString("hex");
const FEDERATION_SERVICE_KEY = crypto.randomBytes(32).toString("hex");
...
async function request(base, pathName, body, key) {
  ...
  try {
    ...
    throw new Error(
      `non_json_response:${pathName}:${response.status}:${responseText.slice(0, 500)}`
    );
  }
...
test("Core attaches a signed V2 preview only after V1 routing and preserves V1 without the preview flag", async () => {
  ...
  process.env.CORE_SERVICE_ADMIN_KEY = crypto.randomBytes(32).toString("hex");
  ...
  try {
    ...
    assert.equal(captured.workPreflight.request.operation_type, "nyra_v2_preview");
    assert.equal(captured.workPreflight.state, "ready_read_only");
    assert.equal(captured.workPreflight.governance.execution_allowed_by_preflight, true);
...
test("Core opens a requested V2 branch and uses Core-validated evidence to evaluate all L2-to-L4 nodes", async () => {
  ...
  const coreEnvironment = {
    ...
    CORE_NYRA_DEEP_BRANCH_V2_LEDGER_SECRET: LEDGER_SECRET,
  ...
  let sourceExcerptB = "Core reviewed official source B evidence excerpt.";
  const { app } = createUniversalCoreService({
  ...
  try {
    ...
    const requiredClaims = [...new Set(
      requirementsRuntime.requirements.map((item) => item.required_claim),
    )];
    const reviewedExcerpt = `Core-reviewed evidence: ${requiredClaims.join(" ")}`;
    assert.ok(reviewedExcerpt.length <= 1_200);
    sourceExcerptA = reviewedExcerpt;
    sourceExcerptB = reviewedExcerpt;
    ...
    const evidencePack = {
      ...
      claims: requirementsRuntime.requirements.map((requirement, index) => ({
        id: `claim_${index + 1}`,
        kind: "fact",
        text: requirement.required_claim,
        facts: [requirement.required_claim],
        claim_hash: requirement.semantic_claim_hash,
        semantic_hash: requirement.semantic_claim_hash,
        content_tag: requirement.required_content_tag,
        capability_spec_hash: requirement.capability_spec_hash,
    ...
    const genericEvidencePack = {
      ...evidencePack,
      claims: evidencePack.claims.map((claim, index) => ({
        ...claim,
        text: `Generic signed evidence claim ${index + 1}.`,
        facts: [`Generic signed evidence claim ${index + 1}.`],
      })),
    };
    const genericEvidencePackHash = nyraDeepV2EvidencePackHash(
      genericEvidencePack,
      requirementBindings,
    );
    const genericRequestId = "nyra-v2-operational-generic-evidence";
    const genericResponse = await request(server.base, "/v1/nira/core-bridge", {
      text: "Reject semantically unrelated signed evidence",
      ...
      deep_branch_v2: {
        ...
        branch_id: branchId,
        subbranch_id: subbranchId,
        evidence_pack: genericEvidencePack,
        requirement_bindings: requirementBindings,
        evidence_pack_hash: genericEvidencePackHash,
        evidence_refs: [],
        request_attestation: signedMcpRequest({
          tenantId,
          ...
          branchId,
          subbranchId,
          evidencePackHash: genericEvidencePackHash,
        }),
      },
    }, key);
    assert.equal(genericResponse.status, 200);
    assert.equal(
      genericResponse.json.result.deep_branch_v2.state,
      "evidence_rejected_v1_authoritative",
    );
    assert.equal(
      genericResponse.json.result.deep_branch_v2.reason,
      "nyra_deep_branch_v2_evidence_acceptance_contract_rejected",
    );
    ...
    assert.equal(JSON.stringify(preparedRuntime).includes("Core-reviewed evidence"), false);
    ...
    assert.equal(
      evaluatedRuntime.state,
      "operational_advisory_verified_v1_authoritative",
      JSON.stringify(evaluatedRuntime),
    );
