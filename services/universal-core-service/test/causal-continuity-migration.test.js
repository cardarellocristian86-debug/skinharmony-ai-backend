...
test("bootstrap compatibility migration preserves bootstrap rows while providing a causal-safe shared ledger", async () => {
  ...
  for (const column of ["sql_digest", "application_state", "checkpoint"]) {
...
test("PostgreSQL shared migration ledger supports bootstrap-first and causal-first initialization", {
  ...
  await withSchema(async (pool) => {
    // This is the pre-hotfix causal-first shape: Bootstrap used to fail with
    // 23502 before it could reconcile the shared ledger.
    await pool.query(`CREATE TABLE core_schema_migrations (
