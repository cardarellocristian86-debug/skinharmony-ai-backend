...
test("verified codexai owner profile activates the pinned 74 advisory branches without execution authority", () => {
  ...
  assert.equal(profile.advisory_activation.active_branch_count, 74);
  assert.equal(profile.advisory_activation.active_branch_digest, OWNER_ACTIVE_ADVISORY_EXPECTED_DIGEST);
  assert.equal(profile.allowed_branches.length, 74);
