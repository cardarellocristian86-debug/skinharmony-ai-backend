...
import { encodeAgentListCursor, normalizeAgentListArgs } from "./agent-list-pagination.js";
...
export function createCollaborationPostgresStore(config, options = {}) {
  ...
  const pool = providedPool || new Pool(postgresPoolConfig(config, {
    connectionString: config.collaborationDatabaseUrl,
    ssl: config.collaborationDatabaseSsl,
  }));
  ...
  function initialize() {
    ...
    const guarded = attempt.then(() => {
      ...
      initializationError = String(error?.code || "collaboration_postgres_initialization_failed").slice(0, 80);
  ...
  function requireInitialized() {
    ...
    const error = new Error("collaboration_postgres_not_ready");
    error.code = "collaboration_postgres_not_ready";
    error.status = 503;
  ...
  const governed = async (identity, action, callback, recovery = null) => {
    // Bootstrap owns schema initialization. Fail before requesting or
    // consuming any Core authorization receipt while the store is cold.
    requireInitialized();
  ...
  return {
    ...
    async listAgents(args, identity) {
      requireInitialized();
      const { limit, cursor } = normalizeAgentListArgs(args);
      const rows = await pool.query(`SELECT agent_id,client_type,display_name,capabilities,last_seen_at,expires_at,version,
        CASE WHEN expires_at > now() THEN 'online' ELSE 'offline' END AS status
        FROM agent_presence
        WHERE tenant_id=$1
          AND ($2::varchar = '' OR agent_id COLLATE "C" < $2::varchar COLLATE "C")
        ORDER BY agent_id COLLATE "C" DESC LIMIT $3`, [
        identity.tenantId,
        cursor?.agent_id || "",
        limit + 1,
      ]);
