...
export function createCollaborationPostgresStore(config, options = {}) {
  ...
  return {
    ...
    async postMessage(args, identity) { const sender=id(args.from_agent_id,"agent"), recipient=args.to_agent_id === "all" ? "all" : id(args.to_agent_id,"agent"), body=text(args.body,"message_body"); const key=String(args.idempotency_key||"").slice(0,120)||null; return governed(identity,boundedCollaborationAction({action_type:"message.post",operation_class:"owner_confirmed_governed_action",contains_customer_data:true,action_label:`Post agent message from ${sender} to ${recipient}`,target:recipient}),async(client)=>{await requireOwnedAgentSession(client,identity,sender);const row=await client.query("INSERT INTO agent_messages (tenant_id,id,thread_id,from_agent_id,to_agent_id,body,idempotency_key) VALUES ($1,gen_random_uuid(),$2,$3,$4,$5,$6) ON CONFLICT (tenant_id,from_agent_id,idempotency_key) DO UPDATE SET id=agent_messages.id RETURNING *",[identity.tenantId,String(args.thread_id||crypto.randomUUID()).slice(0,80),sender,recipient,body,key]); if(recipient!=="all")await client.query("INSERT INTO agent_message_deliveries (tenant_id,message_id,agent_id) VALUES ($1,$2,$3) ON CONFLICT DO NOTHING",[identity.tenantId,row.rows[0].id,recipient]);return {message:row.rows[0]};}); },
