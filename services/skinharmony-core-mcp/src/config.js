...
export function loadConfig(env = process.env) {
  ...
  if ((environmentRoutingRequired || environmentDelegationReceiverEnabled) && Buffer.byteLength(environmentDelegationKey, "utf8") < 32) {
    throw new Error("MCP_ENVIRONMENT_DELEGATION_KEY must contain at least 32 bytes when environment routing is enabled");
  }
