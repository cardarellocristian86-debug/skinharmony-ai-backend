...
export function createCoreHandlers(config, options = {}) {
  ...
  return {
    ...
    software_intelligence: async (args, identity) => textResult(await coreRequest("/v1/software-intelligence/analyze", identity.tenantId, {
      method: "POST",
      body: { artifact: args.artifact, authorization: args.authorization, options: args.options, tenant_id: identity.tenantId },
    })),
    software_authorize: async (args, identity) => {
      const sharedContext = await memoryContext({
        query: "Governed Ghidra or Frida software analysis authorization",
        project_id: args.project_id,
        session_id: args.session_id,
        agent_id: args.agent_id || "connected_ai",
      }, identity);
      const { memory_context: _untrustedMemoryContext, ...safeArgs } = args;
      return textResult(await coreRequest("/v1/software-intelligence/authorize", identity.tenantId, {
        method: "POST",
        body: { ...safeArgs, ...(sharedContext ? { memory_context: sharedContext } : {}), tenant_id: identity.tenantId },
      }));
    },
    ...
    software_job_status: async (args, identity) => textResult(await coreRequest(`/v1/software-intelligence/jobs/${encodeURIComponent(args.job_id)}`, identity.tenantId)),
    software_evidence_get: async (args, identity) => textResult(await coreRequest(`/v1/software-intelligence/jobs/${encodeURIComponent(args.job_id)}`, identity.tenantId)),
    ...
    skin_analyzer: async (args, identity) => textResult(await coreRequest("/v1/branches/skinharmony_analyzer/analyze", identity.tenantId, {
      method: "POST",
      body: {
        data: {
          scores: args.scores,
