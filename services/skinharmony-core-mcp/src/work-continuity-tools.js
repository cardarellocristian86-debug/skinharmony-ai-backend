...
export const WORK_CONTINUITY_TOOLS = [
  ...
  tool("tenant_work_open_review", "Review open Work conflicts",
    ...
    ["request", "intent_type", "create_request"]), false,
    { ownerConfirmationRequired: false, boundedCollaboration: true }),
  ...
  tool("work_continuity_closure_evaluate", "Evaluate independent closure",
    "Evaluate persisted native tasks and independently verified evidence, resolving the release tuple server-side from an exact causal Project/Work/Change and pull-request lookup before requesting Core Join.",
    object({
      work_id: uuid,
      plan_id: uuid,
      release_lookup: object({
        project_id: uuid,
        project_state_digest: hash,
        change_id: uuid,
        pull_request: { type: "integer", minimum: 1 },
      }, ["project_id", "project_state_digest", "change_id", "pull_request"]),
      idempotency_key: text(160),
    }, ["work_id", "plan_id", "release_lookup", "idempotency_key"]),
    false, { ownerConfirmationRequired: false }),
