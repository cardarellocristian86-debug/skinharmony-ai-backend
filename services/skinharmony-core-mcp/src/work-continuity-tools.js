...
export const WORK_CONTINUITY_TOOLS = [
  ...
  tool("work_continuity_v2_create", "Create Work Identity V2",
    ...
    object({
      ...
      project_id: identifier, work_id: uuid, parent_work_id: uuid,
      session_id: identifier, work_name: text(1_000),
  ...
  tool("tenant_work_historical_archive_v3", "Archive an uncloseable historical bridged Work",
    "Owner-confirm and archive one stale V2 Work linked to legacy continuity without claiming it completed or changing the legacy record. A BLOCKED_VALID bridge may instead be bound to an exact same-project successor with a server-persisted closure receipt and final report. An explicit owner may revoke only pre-attestation read-only bindings. Branches and execution-capable leases always block, except an explicit opt-in may retire an exact, untouched set of Nyra autopilot bootstrap branches and cancel only their unclaimed offers. A timestamp-repair archive is permitted only for an old, fully verified V2 BLOCKED/release_ready bridge when a prior unsigned diagnostic read refreshed its legacy timestamp.",
  ...
  tool("tenant_work_task_contract_record", "Record governed Task State Contract",
    "Append one versioned task contract bound to the immutable Work Intent. The contract declares inputs, dependencies, required claims, allowed effects, recovery policy and budgets; it grants no authority.",
    object({
      work_id: uuid, task_id: uuid,
      contract_revision: { type: "integer", minimum: 1, maximum: 1_000_000 },
      intent_digest: hash,
      declared_inputs: { type: "object", additionalProperties: true },
      dependency_refs: { type: "array", maxItems: 250, uniqueItems: true, items: { type: "string", minLength: 1, maxLength: 500 } },
      output_schema_ref: { type: "string", minLength: 2, maxLength: 500, pattern: "^[a-zA-Z0-9][a-zA-Z0-9._:/-]+$" },
      required_claims: { type: "array", maxItems: 100, uniqueItems: true, items: text(240) },
      allowed_effects: { type: "array", maxItems: 100, uniqueItems: true, items: identifier },
      recovery_policy: { type: "object", additionalProperties: true },
      budgets: { type: "object", additionalProperties: true },
    }, ["work_id", "task_id", "contract_revision", "intent_digest", "declared_inputs",
      "dependency_refs", "output_schema_ref", "required_claims", "allowed_effects",
      "recovery_policy", "budgets"]), false,
    { ownerConfirmationRequired: false, boundedCollaboration: true }),
  tool("tenant_work_dependency_manifest_record", "Record validated dependency manifest",
    "Append a server-digested plan dependency manifest. Mandatory task dependencies are server-checked and omission or stale task/Intent state fails closed.",
    object({
      work_id: uuid, task_id: uuid,
      expected_manifest_revision: { type: "integer", minimum: 0, maximum: 1_000_000 },
      plan_digest: hash,
      task_revision: { type: "integer", minimum: 1, maximum: 1_000_000 },
      intent_digest: hash,
      dependency_ids: { type: "array", maxItems: 250, uniqueItems: true, items: identifier },
      source_versions: { type: "object", additionalProperties: true },
      relevant_predicates: { type: "object", additionalProperties: true },
      required_evidence_refs: { type: "array", maxItems: 250, uniqueItems: true, items: identifier },
      policy_revision: hash,
      idempotency_key: coordinationIdempotencyKey,
    }, ["work_id", "task_id", "expected_manifest_revision", "plan_digest",
      "task_revision", "intent_digest", "dependency_ids", "source_versions",
      "relevant_predicates", "required_evidence_refs", "policy_revision", "idempotency_key"]),
    false, { ownerConfirmationRequired: false, boundedCollaboration: true }),
  tool("tenant_work_trajectory_evaluate", "Persist cumulative Work trajectory",
    "Atomically fold bounded read/write scopes, recipients, effects and egress across agents and harnesses. The result is a non-authorizing Core signal and HOLD survives session changes.",
    object({
      work_id: uuid,
      expected_trajectory_revision: { type: "integer", minimum: 0, maximum: 1_000_000 },
      harness_digest: hash,
      proposal: object({
        read_scopes: { type: "array", maxItems: 1_000, uniqueItems: true, items: identifier },
        write_scopes: { type: "array", maxItems: 1_000, uniqueItems: true, items: identifier },
        recipients: { type: "array", maxItems: 1_000, uniqueItems: true, items: identifier },
        effect_count: { type: "integer", minimum: 0, maximum: 1_000 },
        egress_count: { type: "integer", minimum: 0, maximum: 1_000 },
      }, ["read_scopes", "write_scopes", "recipients", "effect_count", "egress_count"]),
      policy: object({
        max_read_scopes: { type: "integer", minimum: 0, maximum: 1_000_000 },
        max_write_scopes: { type: "integer", minimum: 0, maximum: 1_000_000 },
        max_recipients: { type: "integer", minimum: 0, maximum: 1_000_000 },
        max_effects: { type: "integer", minimum: 0, maximum: 1_000_000 },
        max_egress: { type: "integer", minimum: 0, maximum: 1_000_000 },
        forbidden_scope_pairs: { type: "array", maxItems: 100,
          items: { type: "array", minItems: 2, maxItems: 2, items: identifier } },
      }, ["max_read_scopes", "max_write_scopes", "max_recipients", "max_effects",
        "max_egress", "forbidden_scope_pairs"]),
      idempotency_key: coordinationIdempotencyKey,
    }, ["work_id", "expected_trajectory_revision", "harness_digest", "proposal", "policy",
      "idempotency_key"]), false,
    { ownerConfirmationRequired: false, boundedCollaboration: true }),
  tool("tenant_work_task_commit", "Commit validated task state",
    "Atomically commit only independently verified task output, its evidence and effect lineage with task CAS, an append-only Ledger event and deterministic Work projection update.",
    object({
      work_id: uuid, task_id: uuid,
      expected_task_revision: { type: "integer", minimum: 1, maximum: 1_000_000 },
      contract_revision: { type: "integer", minimum: 1, maximum: 1_000_000 },
      input_digest: hash,
      output_ref: { type: "string", minLength: 2, maxLength: 500, pattern: "^[a-zA-Z0-9][a-zA-Z0-9._:/-]+$" },
      output_digest: hash,
      evidence_refs: { type: "array", maxItems: 250, uniqueItems: true, items: uuid },
      effect_lineage_refs: { type: "array", maxItems: 250, uniqueItems: true, items: identifier },
      dependency_manifest_digest: hash,
      validation_ref: { type: "string", minLength: 2, maxLength: 500, pattern: "^[a-zA-Z0-9][a-zA-Z0-9._:/-]+$" },
      idempotency_key: coordinationIdempotencyKey,
    }, ["work_id", "task_id", "expected_task_revision", "contract_revision", "input_digest",
      "output_ref", "output_digest", "evidence_refs", "effect_lineage_refs",
      "dependency_manifest_digest", "validation_ref",
      "idempotency_key"]), false,
    { ownerConfirmationRequired: false, boundedCollaboration: true }),
  tool("tenant_work_task_invalidate", "Invalidate committed task dependencies",
    "Append an explicit invalidation when a committed task dependency materially changes. History is preserved and unrelated dependency drift is rejected.",
    object({
      work_id: uuid, task_id: uuid,
      changed_dependency_refs: { type: "array", minItems: 1, maxItems: 250, uniqueItems: true, items: { type: "string", minLength: 1, maxLength: 500 } },
      reason: text(500), idempotency_key: coordinationIdempotencyKey,
    }, ["work_id", "task_id", "changed_dependency_refs", "reason", "idempotency_key"]), false,
    { ownerConfirmationRequired: false, boundedCollaboration: true }),
  tool("tenant_work_state_projection_read", "Read deterministic Work state projection",
    "Read the tenant-authorized incremental projection and catch it up in memory from only Ledger events after its watermark. Optional agent, verifier, Core and Owner views are authorized server-side and expose only their relevant state. The read never persists, grants authority or requires a mutation ticket.",
    object({ work_id: uuid, view: { type: "string", enum: ["agent", "verifier", "core", "owner"] } }, ["work_id"]), true),
  tool("tenant_work_execution_constraints_read", "Read execution constraints projection",
    "Read a server-derived, non-authorizing view of the current runtime, published tool count, policy origin and authenticated effect ceiling. Unobservable network, resources and concurrency remain UNKNOWN.",
    object({ work_id: uuid, task_id: uuid }, ["work_id", "task_id"]), true),
  ...
  tool("tenant_work_legacy_reconcile_close", "Close stale legacy Work honestly",
    ...
    object({
      ...
      expected_classification: { type: "string", enum: ["STALE", "ABANDONED", "BLOCKED_VALID", "COMPLETED_BUT_UNCLOSED"] },
      reason: text(1_000),
      successor_work_id: uuid,
      idempotency_key: coordinationIdempotencyKey,
  ...
  tool("work_continuity_native_acceptance_contract_read", "Read bound verifier acceptance contract",
    ...
    true),
  tool("work_continuity_native_plan_merge_preview", "Preview a server-derived native plan merge",
    "Inspect the immutable native-plan graph, derive structural heads and preview a deterministic three-way merge without changing plans, evidence, authority or Work state.",
