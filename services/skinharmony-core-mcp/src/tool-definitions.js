...
export const TOOLS = [
  ...
  tool("work_continuity_create", "Create persistent work", "Create a tenant-scoped Work Identity, Architecture Map and Continuity Capsule; it never authorizes execution.", object({ project_id: text(160), session_id: { type: "string", maxLength: 160 }, parent_work_id: { type: "string", maxLength: 160 }, next_action: { type: "string", maxLength: 1000 }, architecture_map: { type: "object", properties: { idea_origin: text(1000), objective: text(4000), current_state: { type: "string", maxLength: 160 } }, required: ["idea_origin", "objective"], additionalProperties: true } }, ["project_id", "architecture_map"]), ["core:govern"], false, false),
  tool("work_continuity_read", "Read persistent work", "Read a tenant-scoped Work Architecture, ledger, checkpoint and capsule.", object({ work_id: { type: "string", maxLength: 160 } }, ["work_id"]), ["core:read"]),
  tool("work_continuity_event", "Append continuity event", "Append an idempotent event. A function change may carry its Impact Map.", object({ work_id: { type: "string", maxLength: 160 }, event_type: { type: "string", enum: ["branch_opened", "function_added", "dependency_changed", "test_completed", "defect_found", "correction_verified", "handoff_created"] }, actor: identifier, data: { type: "object", additionalProperties: true }, idempotency_key: text(160) }, ["work_id", "event_type", "actor", "idempotency_key"]), ["core:govern"], false, true),
  tool("work_continuity_checkpoint", "Create continuity checkpoint", "Persist next action, evidence and a verifiable Continuity Capsule.", object({ work_id: { type: "string", maxLength: 160 }, actor: identifier, next_action: text(1000), tests: { type: "array", maxItems: 200, items: { type: "object", additionalProperties: true } }, commit_patch: { type: "object", additionalProperties: true }, rollback: { type: "object", additionalProperties: true }, idempotency_key: text(160) }, ["work_id", "actor", "next_action", "idempotency_key"]), ["core:govern"], false, true),
  tool("work_continuity_resume", "Resume persistent work", "Resume from a verified capsule; drift blocks resume and Core revalidation is required.", object({ work_id: { type: "string", maxLength: 160 }, session_id: text(160), repository_digest: { type: "string", maxLength: 160 }, policy_digest: { type: "string", maxLength: 160 } }, ["work_id", "session_id"]), ["core:read"]),
