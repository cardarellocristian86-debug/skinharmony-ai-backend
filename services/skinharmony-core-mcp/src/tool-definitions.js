...
export const TOOLS = [
  ...
  tool("core_capability_invoke", "Invoke a governed dynamic capability", "Invoke one server-registered mutating capability by exact capability id and catalog revision. Tenant-bounded Gallery coordination and bounded post-delegation actions use verified server-side identity and narrow Core gates; only a target explicitly marked owner-confirmed may consume one fresh owner confirmation.", object({
    capability_id: identifier,
    catalog_revision: { type: "string", pattern: "^[a-f0-9]{64}$" },
    arguments: { type: "object", maxProperties: 200, additionalProperties: true },
    ...
    meta: { "skinharmony/conditionalTargetGovernance": true },
  }),
  ...
  tool("ai_eval_scorecard_read", "Read AI evaluation scorecards", "Read bounded, versioned tenant evaluation scorecards. It exposes redacted metrics and evidence digests only and never promotes a candidate.", object({
    scorecard_id: identifier,
    release_version: { type: "string", maxLength: 120 },
    limit: { type: "integer", minimum: 1, maximum: 100 },
    cursor: { type: "string", maxLength: 160 },
  }), ["core:read"]),
  tool("ai_eval_dataset_read", "Read AI evaluation dataset metadata", "Read tenant-scoped dataset versions, provenance and policy metadata without raw prompts, customer content or secrets.", object({
    dataset_id: identifier,
    version: { type: "string", maxLength: 120 },
    limit: { type: "integer", minimum: 1, maximum: 100 },
    cursor: { type: "string", maxLength: 160 },
  }), ["core:read"]),
  tool("ai_eval_trace_read", "Read redacted AI evaluation traces", "Read bounded redacted trace metadata for evaluation and regression analysis. Raw prompts and sensitive content are excluded by default.", object({
    trace_id: identifier,
    run_id: identifier,
    limit: { type: "integer", minimum: 1, maximum: 100 },
    cursor: { type: "string", maxLength: 160 },
  }), ["core:read"]),
  tool("ai_performance_scorecard_read", "Read AI runtime performance scorecards", "Read p50, p95, p99, TTFT, token, cost and verified-outcome performance metrics without enabling provider execution.", object({
    scorecard_id: identifier,
    release_version: { type: "string", maxLength: 120 },
    limit: { type: "integer", minimum: 1, maximum: 100 },
    cursor: { type: "string", maxLength: 160 },
  ...
  tool("ai_learning_review_binding_preview", "Preview an AI learning review binding", "Build the exact server-derived evidence payload that independent DTT reviewers must attest for either a candidate review or a verified learning outcome. It performs no promotion or execution.", {
    ...object({
      candidate_id: identifier,
      decision: { type: "string", enum: ["approved_for_shadow"] },
  ...
  tool("ai_learning_candidate_review", "Review an AI learning candidate", "Record an owner-confirmed, audited review decision for one versioned candidate. It creates no promotion, provider execution or live mutation.", object({
    ...
    review_attestation: object({
      tree_id: identifier,
      node_id: identifier,
    }, ["tree_id", "node_id"]),
    review_binding_receipt: object({
      schema_version: { type: "string", enum: ["ai_learning_review_binding_receipt_v0_16"] },
      tenant_id: identifier,
      candidate_id: identifier,
      candidate_version: identifier,
      source_revision: { type: "integer", minimum: 1 },
      resulting_revision: { type: "integer", minimum: 2 },
      decision: { type: "string", enum: ["approved_for_shadow"] },
      issued_at: { type: "string", minLength: 20, maxLength: 64 },
      expires_at: { type: "string", minLength: 20, maxLength: 64 },
      nonce: { type: "string", pattern: "^airn_[a-f0-9]{32}$" },
      binding_digest: { type: "string", pattern: "^sha256:[a-f0-9]{64}$" },
      evidence_snapshot_digest: { type: "string", pattern: "^sha256:[a-f0-9]{64}$" },
      signature: { type: "string", pattern: "^airr_[a-f0-9]{64}$" },
    }, [
      "schema_version",
      "tenant_id",
      "candidate_id",
    ...
    expected_revision: { type: "integer", minimum: 1 },
    idempotency_key: { type: "string", minLength: 8, maxLength: 160 },
  }, ["candidate_id", "decision", "review_note", "expected_revision", "idempotency_key"]), ["core:govern"], false, true),
  tool("ai_learning_outcome_record", "Record a governed AI learning outcome", "Record one verified, tenant-scoped learning outcome with provenance and an idempotency key. It never trains, promotes or changes live weights automatically.", object({
    ...
    idempotency_key: { type: "string", minLength: 8, maxLength: 160 },
  }, ["outcome", "expected_revision", "idempotency_key"]), ["core:govern"], false, true),
