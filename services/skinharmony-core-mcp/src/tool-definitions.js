...
const ownerConfirmationProperties = {
  owner_confirmed: {
    type: "boolean",
    description: "Client-side confirmation claim for audit only. The server does not treat this boolean as proof; verified identity or a future signed Core receipt is still required.",
  },
  confirmation_reference: {
    type: "string",
    maxLength: 240,
    description: "Short non-secret reference supplied with the client claim; it is not an authorization credential.",
  },
};
...
export const TOOLS = [
  ...
  tool("render_staging_deploy", "Deploy Universal Core staging", "Trigger one request-bound deploy of an exact commit to the fixed Universal Core staging service. The server verifies the Render service binding, requires auto-deploy off, never changes configuration, and never returns credentials.", object({
    target_service: { type: "string", const: "skinharmony-universal-core-staging" },
    target_environment: { type: "string", const: "staging" },
    target_branch: { type: "string", const: "agent/nyra-policy-registry" },
    target_commit: { type: "string", pattern: "^[a-f0-9]{40}$" },
    clear_build_cache: { type: "boolean", const: false },
    modify_environment_variables: { type: "boolean", const: false },
    database_changes: { type: "boolean", const: false },
    auth0_changes: { type: "boolean", const: false },
    production_deploy: { type: "boolean", const: false },
    merge: { type: "boolean", const: false },
    delete: { type: "boolean", const: false },
    cross_tenant: { type: "boolean", const: false },
  }, [
    "target_service", "target_environment", "target_branch", "target_commit",
    "clear_build_cache", "modify_environment_variables", "database_changes",
    "auth0_changes", "production_deploy", "merge", "delete", "cross_tenant",
  ]), ["core:govern"], false, true, { openWorld: true }),
  tool("render_staging_deploy_status", "Read Universal Core staging deploy", "Read one exact deploy of the fixed Universal Core staging service and verify health only after Render reports it live. It cannot trigger, cancel, restart, modify, or roll back a deploy.", object({
    target_service: { type: "string", const: "skinharmony-universal-core-staging" },
    target_environment: { type: "string", const: "staging" },
    target_commit: { type: "string", pattern: "^[a-f0-9]{40}$" },
  }, ["target_service", "target_environment", "target_commit"]), ["core:read"], true, true, { openWorld: true }),
  ...
  tool("core_gate_action", "Evaluate and authorize a scoped action", "Ask Universal Core to evaluate an action and, only for supported fail-closed operation classes, return a scoped authorization. This tool never executes the action.", {
    type: "object",
    required: ["action_label", "action_type"],
    properties: {
      action_label: text(500),
      action_type: text(120),
      operation_class: text(120),
      domain_action_id: text(120),
      target: { type: "string", maxLength: 500 },
      project_id: identifier,
      target_commit: { type: "string", pattern: "^[a-fA-F0-9]{40}$" },
      read_only: { type: "boolean" },
      dry_run: { type: "boolean" },
      external_side_effect: { type: "boolean" },
      contains_customer_data: { type: "boolean" },
      contains_secret: { type: "boolean" },
      secret_value_transmitted: { type: "boolean" },
      values_present_in_envelope: { type: "boolean" },
      cross_tenant: { type: "boolean" },
      destructive: { type: "boolean" },
      verified_outcome: { type: "boolean" },
      bypass_orchestrator: { type: "boolean" },
      rollback_ready: { type: "boolean" },
      audit_ready: { type: "boolean" },
      configuration_changes: { type: "boolean" },
      provider_execution: { type: "boolean" },
      execution_enabled: { type: "boolean" },
      deploy: { type: "boolean" },
      database_connection: { type: "boolean" },
      database_mutation: { type: "boolean" },
      readback_required: { type: "boolean" },
      auth0_changes: { type: "boolean" },
      production_deploy: { type: "boolean" },
      merge: { type: "boolean" },
      delete: { type: "boolean" },
      create_missing_only: { type: "boolean" },
      overwrite_existing: { type: "boolean" },
      target_branch: { type: "string", maxLength: 240 },
      ...
      topology: { type: "object", maxProperties: 16, additionalProperties: true },
      provider_native_references: { type: "boolean" },
      secret_values_present: { type: "boolean" },
      spec_digest: { type: "string", pattern: "^[a-f0-9]{64}$" },
      confirmation_spec_digest: { type: "string", pattern: "^[a-f0-9]{64}$" },
      maximum_recurring_monthly_cost_cents: { type: "integer", minimum: 1, maximum: 10_000 },
      recurring_cost_currency: { type: "string", enum: ["USD"] },
      recurring_cost_confirmed: { type: "boolean" },
      confirmation_maximum_recurring_monthly_cost_cents: {
        type: "integer",
        minimum: 1,
        maximum: 10_000,
      },
      confirmation_recurring_cost_currency: { type: "string", enum: ["USD"] },
    },
    additionalProperties: false,
  }, ["core:govern"], false, true),
  ...
  tool("workspace_create_folder", "Create shared folder", "Create a tenant-scoped logical folder after Core governance. PostgreSQL mode requires an idempotency key and an owned parent-or-equal path lock with its fencing token.", object({ path: text(240), idempotency_key: text(120), lock_id: { type: "string", format: "uuid" }, fencing_token: { type: "integer", minimum: 1 } }, ["path"]), ["core:govern"], false, true),
  ...
  tool("workspace_write_document", "Write shared document", "Create or version a tenant-scoped document with optimistic concurrency and Core governance. PostgreSQL mode requires expected_version, idempotency_key, an active lock_id and its fencing_token.", object({ path: text(240), title: { type: "string", maxLength: 200 }, content: text(100_000), expected_version: { type: "integer", minimum: 0 }, idempotency_key: { type: "string", maxLength: 120 }, lock_id: { type: "string", format: "uuid" }, fencing_token: { type: "integer", minimum: 1 } }, ["path", "content"]), ["core:govern"], false, true),
  tool("workspace_lock_acquire", "Acquire shared workspace lock", "Acquire an exclusive tenant-scoped path lease before editing a document. Equal, parent and child paths conflict; the returned fencing token must accompany the write.", object({ path: text(240), lease_seconds: { type: "integer", minimum: 30, maximum: 300 }, idempotency_key: text(120) }, ["path", "idempotency_key"]), ["core:govern"], false, true),
  tool("workspace_lock_renew", "Renew shared workspace lock", "Renew an active path lease owned by the current signed agent session.", object({ lock_id: { type: "string", format: "uuid" }, fencing_token: { type: "integer", minimum: 1 }, lease_seconds: { type: "integer", minimum: 30, maximum: 300 }, idempotency_key: text(120) }, ["lock_id", "fencing_token", "idempotency_key"]), ["core:govern"], false, true),
  tool("workspace_lock_release", "Release shared workspace lock", "Release only the active path lease owned by the current signed agent session. The audit row is preserved.", object({ lock_id: { type: "string", format: "uuid" }, fencing_token: { type: "integer", minimum: 1 } }, ["lock_id", "fencing_token"]), ["core:govern"], false, true),
  tool("workspace_lock_list", "List shared workspace locks", "Read active tenant-scoped path leases, optionally including expired or released audit records.", object({ prefix: { type: "string", maxLength: 240 }, include_inactive: { type: "boolean" }, limit: { type: "integer", minimum: 1, maximum: 100 } }), ["core:read"]),
  ...
  tool("task_claim", "Claim shared task", "Atomically claim an open tenant-scoped task for one registered agent.", object({ task_id: text(80), agent_id: identifier, expected_version: { type: "integer", minimum: 1 } }, ["task_id", "agent_id", "expected_version"]), ["core:govern"], false, false),
  tool("task_update", "Update shared task", "Update a claimed tenant-scoped task using optimistic concurrency. PostgreSQL mode also requires the lease_token and fencing_token returned by task_claim.", object({ task_id: text(80), agent_id: identifier, status: { type: "string", enum: ["claimed", "in_progress", "blocked", "completed", "cancelled"] }, note: { type: "string", maxLength: 10_000 }, expected_version: { type: "integer", minimum: 1 }, lease_token: { type: "string", format: "uuid" }, fencing_token: { type: "integer", minimum: 1 } }, ["task_id", "agent_id", "status", "expected_version"]), ["core:govern"], false, false),

  tool("agent_heartbeat", "Register unique agent presence", "Register or refresh one uniquely signed ChatGPT, Codex, API-agent or other session. A different session cannot silently reuse the same agent_id.", object({ agent_id: identifier, client_type: { type: "string", enum: ["chatgpt", "codex", "api_agent", "other"] }, session_id: { type: "string", minLength: 1, maxLength: 240 }, display_name: { type: "string", maxLength: 120 }, capabilities: { type: "array", maxItems: 20, items: identifier } }, ["agent_id", "client_type", "session_id"]), ["core:govern"], false, false),
