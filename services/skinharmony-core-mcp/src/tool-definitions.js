...
export const TOOLS = [
  ...
  tool("core_gate_action", "Evaluate and authorize a scoped action", "Ask Universal Core to evaluate an action and, only for supported fail-closed operation classes, return a scoped authorization. This tool never executes the action.", { type: "object", required: ["action_label", "action_type"], properties: { action_label: text(500), action_type: text(120), operation_class: text(120), target: text(240), target_branch: text(240), target_commit: { type: "string", pattern: "^[a-f0-9]{40}$" }, confirmed_action_digest: { type: "string", pattern: "^[a-f0-9]{64}$", description: "Digest returned by a prior exact Core gate evaluation and explicitly confirmed by the owner." }, deployment_spec: { type: "object", additionalProperties: true, description: "Domain-adapter deployment specification. The selected Core adapter applies a closed exact schema." }, read_only: { type: "boolean" }, dry_run: { type: "boolean" }, external_side_effect: { type: "boolean" }, contains_customer_data: { type: "boolean" }, contains_secret: { type: "boolean" }, cross_tenant: { type: "boolean" }, destructive: { type: "boolean" }, verified_outcome: { type: "boolean" }, bypass_orchestrator: { type: "boolean" }, rollback_ready: { type: "boolean" }, audit_ready: { type: "boolean" }, configuration_changes: { type: "boolean" } }, additionalProperties: true }, ["core:govern"], false, true),
