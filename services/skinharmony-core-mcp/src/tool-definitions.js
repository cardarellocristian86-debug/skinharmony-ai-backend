...
export const TOOLS = [
  ...
  tool("core_key_presets", "Read key presets", "Read the public key presets and supported key types so the UI can reuse an existing bootstrap path instead of inventing new keys.", object(), ["core:read"]),
  tool("core_key_generate", "Generate a tenant key", "Mint a tenant-scoped API key using the authenticated admin channel. Use this only when the UI needs a durable reusable connector key.", object({
    tenant_id: identifier,
    key_type: { type: "string", maxLength: 80 },
    preset: { type: "string", maxLength: 80 },
    label: { type: "string", maxLength: 240 },
    tier: { type: "string", maxLength: 80 },
    suite_tier: { type: "string", maxLength: 80 },
    allowed_scopes: { type: "array", maxItems: 50, items: { type: "string", maxLength: 80 } },
    active_branches: { type: "array", maxItems: 50, items: identifier },
    suite_modules: { type: "array", maxItems: 50, items: identifier },
    suite_limits: { type: "object", additionalProperties: true },
    brand_scope: { type: "string", maxLength: 120 },
    expires_at: { type: "string", maxLength: 40 },
    metadata: { type: "object", additionalProperties: true },
  }, ["tenant_id"]), ["core:govern"], false, true),
  tool("core_setup_token_create", "Create a one-time setup token", "Create a one-time bootstrap token for a tenant so the UI can mint a reusable key once, then store that key in the connector or env.", object({
    tenant_id: identifier,
    tenant: { type: "object", additionalProperties: true },
    brand_scope: { type: "string", maxLength: 120 },
    environment: { type: "string", maxLength: 80 },
    branch_groups: { type: "array", maxItems: 50, items: identifier },
    branches: { type: "array", maxItems: 50, items: identifier },
    active_branch_groups: { type: "array", maxItems: 50, items: identifier },
    active_branches: { type: "array", maxItems: 50, items: identifier },
    plan: { type: "string", maxLength: 80 },
    key_type: { type: "string", maxLength: 80 },
    preset: { type: "string", maxLength: 80 },
    label: { type: "string", maxLength: 240 },
    role: { type: "string", maxLength: 80 },
    scopes: { type: "array", maxItems: 50, items: { type: "string", maxLength: 80 } },
    modules: { type: "array", maxItems: 50, items: identifier },
    limits: { type: "object", additionalProperties: true },
    gate_mode: { type: "string", maxLength: 80 },
    recommended_folders: { type: "array", maxItems: 20, items: { type: "string", maxLength: 240 } },
    policy: { type: "object", additionalProperties: true },
    metadata: { type: "object", additionalProperties: true },
    key_expires_at: { type: "string", maxLength: 40 },
  }, ["tenant_id"]), ["core:govern"], false, true),
  tool("core_setup_token_consume", "Consume a setup token", "Consume a one-time setup token and receive the reusable API key plus bootstrap profile.", object({
    setup_token: { type: "string", maxLength: 240 },
    token: { type: "string", maxLength: 240 },
    actor_id: identifier,
    connector: { type: "string", maxLength: 120 },
