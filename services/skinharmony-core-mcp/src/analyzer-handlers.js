...
function professionalProfile(value) {
  ...
  const profiles = {
    medical_study: {
      ...
      language: "quantitative_observations_with_clinical_correlation_required",
      allowed: ["acquisition_quality", "zone_measurements", "longitudinal_deltas", "uncertainty", "review_questions"],
      blocked: ["ai_diagnosis", "ai_differential_diagnosis", "treatment_plan", "prescription"],
    },
    salon_trichology: {
      label: "Parrucchiere / tecnico tricologico",
      output_mode: "technical_cosmetic_consultation",
      audience: "trained_hair_professional",
      language: "technical_observational_cosmetic",
      allowed: ["surface_balance", "shaft_quality", "zone_comparison", "cosmetic_protocol_direction", "follow_up"],
      blocked: ["diagnosis", "disease_name", "hair_loss_cause", "therapy", "prescription"],
    },
    pharmacy_dermocosmetic: {
      ...
      language: "dermocosmetic_tolerability_and_adherence",
      allowed: ["surface_balance", "tolerability_questions", "ingredient_family_direction", "routine_adherence", "follow_up"],
      blocked: ["diagnosis", "drug_recommendation", "prescription", "therapeutic_claim"],
    },
  };
...
function communicationPack(profile, dominant, abstain, stop) {
  const pattern = dominant?.id || "data_quality";
  const patternLabels = {
    density_distribution: "distribuzione visiva e uniformita delle aree osservate",
    shaft_variability: "qualita visiva e variabilita dei fusti",
    surface_balance: "equilibrio cosmetico della superficie del cuoio capelluto",
    follicular_units: "distribuzione visiva delle unita follicolari",
    data_quality: "qualita dell'acquisizione",
  };
  const focus = patternLabels[pattern] || patternLabels.data_quality;
  const base = {
...
function interpretScalp(args = {}) {
  ...
  return {
    ...
    longitudinal: {
      ...comparison,
      deltas: comparison.comparable && previousOverall ? metricDeltas(overall, previousOverall) : {},
    ...
    learning: {
      eligible_candidate: Boolean(learningEligible),
      activation_allowed: false,
      requires: ["verified_outcome", "human_review", "comparable_capture_series", "core_regression_gate"],
      reason: learningEligible ? "verified_candidate_ready_for_core_review" : "insufficient_verified_comparable_evidence",
    },
    professional_context: {
      ...profile,
      ai_role: profile.id === "medical_study" ? "measurement_and_documentation_support" : "cosmetic_observation_support",
      clinician_review_required: profile.id === "medical_study",
