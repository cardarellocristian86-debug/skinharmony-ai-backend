...
7. **Provider setup.** ChatGPT Pro/Codex and API billing are separate. Never paste an API key into ChatGPT. Choose **Collega OpenAI**: the fixed page asks for OAuth owner login, then creates a short-lived, one-time protected page where the user pastes the existing key. The chat never receives the key or the one-time credential. The user sees only a masked status and can rotate or remove it.
...
The fixed page `https://skinharmony-core-mcp.onrender.com/connect/openai` never accepts a tenant identifier. It uses Authorization Code + PKCE, verifies the authenticated OAuth `owner_root`, exact owner subject allowlist and tenant claim, then asks Core to issue the one-time vault page. A Codex bearer token and an OAuth client id alone cannot authorize this credential flow. The setup URL has no tenant or key; its short-lived path token is paired with a separate proof kept only in the URL fragment. Core consumes that proof and persists the encrypted key in one database transaction, so a failed or revoked flow does not partially change the provider configuration. It never enables provider execution.
...
Set the same high-entropy `CORE_OWNER_CONTEXT_SIGNING_SECRET` of at least 32 characters manually on **both** `skinharmony-core-mcp` and `skinharmony-universal-core`. It signs only the short-lived OAuth-owner confirmation for the exact provider-link binding; it is not an OpenAI key, not a Core bearer key and must never be sent in chat, a URL or the repository. If it is absent, weak or differs between services, the flow fails closed and no provider key is stored. The binding commit is taken from the full Render build SHA (`RENDER_GIT_COMMIT`) of the currently running MCP process, never from the caller; if that identity is unavailable, the gate blocks. This Core gate is an authorization and audit decision, not a Render deployment executor: Blueprint sync is physically enforced by Render/GitHub until a separate CI/Render executor verifies Core's signed approval.

The one-time-link request uses a separate server-side credential, `CORE_PROVIDER_SETUP_LINK_KEY`, generated by Render and bound only to `MCP_CHATGPT_TENANT_ID`. It is used only for `POST /v1/generic-agents/providers/openai/setup-links`; it never falls back to `CORE_MCP_KEY`, `UNIVERSAL_CORE_KEY`, or the normal tenant key map. Core accepts it only as its dedicated provider-link issuer and additionally verifies the signed OAuth owner context. For multiple intentionally provisioned tenants, use `UNIVERSAL_CORE_PROVIDER_SETUP_LINK_KEYS_JSON`; every entry remains tenant-bound and must carry only the Core `write:provider_setup_link` scope. Direct bearer-key provider write/delete routes are disabled: the consumed one-time page is the only credential-entry channel.
