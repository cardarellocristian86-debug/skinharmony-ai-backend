...
Quando `DATABASE_URL` e configurato, ogni chiamata MCP autenticata viene
registrata automaticamente in PostgreSQL. Il client AI non puo scegliere il
tenant e non deve ricordarsi volontariamente di scrivere il report.

Il ledger conserva sessioni di lavoro, preflight, interventi Core, negazioni,
richieste di conferma, errori e outcome verificati. Gli eventi sono append-only,
concatenati con SHA-256 e serializzati per lavoro tramite advisory lock. Prompt,
risposte integrali e segreti non vengono conservati: restano sintesi redatte,
hash, reason code, versioni e riferimenti alle evidenze.

In produzione `CORE_DECISION_LEDGER_REQUIRED=true` rende PostgreSQL obbligatorio:
se il lavoro non puo essere registrato, il Gateway non avvia la tool call. Il
tool read-only `decision_ledger_report` restituisce metriche esclusivamente per
il tenant autenticato.
...
La versione `0.8.0` espone a ChatGPT un ciclo analitico completo,
...
## Software intelligence and analyzer tools

`software_intelligence` performs immediate authorized static inspection. Governed
asynchronous work uses `software_authorize`, `software_job_submit`,
`software_job_status` and `software_evidence_get`; the historical
`software_job_get` remains available with the same behavior. Deep Ghidra and
Frida modes fail closed without owner confirmation, a short-lived signed Core
envelope, a sandbox, and (for Frida) an allowlisted target and fixed template.
Completed jobs expose redacted `universal_software_evidence_v1` evidence and do
not persist raw artifacts.

`skin_analyzer` routes structured scores to the analyzer branch selected by the
authenticated Core key. `scalp_analyzer` interprets structured local metrics in
the MCP adapter. Both are read-only, non-diagnostic and automatically pass the
mandatory memory-first preflight. They never accept `tenant_id`; raw analyzer
images, prescriptions and medical claims are outside the tool contract.

Every advertised tool has an `inputSchema`, `outputSchema`, usage-oriented
description, OAuth scopes and impact annotations. Invalid or cross-tenant-shaped
arguments are rejected before execution. Governed backend rejections are
returned as safe MCP tool errors with stable codes and retry guidance.
