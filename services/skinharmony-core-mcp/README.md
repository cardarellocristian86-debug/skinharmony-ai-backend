...
## iPhone multi-agent portal

Custom MCP applications are not a dependable write surface in the native
ChatGPT mobile app. Tenant owners can instead open the mobile-first Safari
portal at `https://skinharmony-core-mcp.onrender.com/mobile/agents`. It reuses
the same Auth0 Authorization Code + PKCE login, encrypted tenant vault and
bounded Core runner; it does not require Render, a Mac or a terminal.
