...
test("maps the complete intelligence toolset to tenant-scoped Core routes", async () => {
  ...
  await handlers.software_intelligence({ artifact: { name: "x", content_base64: "AA==" }, authorization: { asserted: true, basis: "owned", purpose: "testing" } }, identity);
  ...
  await handlers.software_job_status({ job_id: "usij_00000000-0000-4000-8000-000000000000" }, identity);
  await handlers.software_evidence_get({ job_id: "usij_00000000-0000-4000-8000-000000000000" }, identity);
  ...
  await handlers.skin_analyzer({ scores: [{ key: "pores_texture", score: 42 }] }, identity);
  assert.deepEqual(calls.map((call) => new URL(call.url).pathname), [
    ...
    "/v1/software-intelligence/analyze",
    ...
    "/v1/branches/skinharmony_analyzer/analyze",
  ]);
  assert(calls.every((call) => call.init.headers.authorization === "Bearer tenant-a-key"));
  ...
  assert.equal(JSON.parse(calls[12].init.body).memory_context.tenant_id, "tenant-a");
  assert.equal(JSON.parse(calls.at(-1).init.body).tenant_id, "tenant-a");
