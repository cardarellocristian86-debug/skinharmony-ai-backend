...
test("maps MCP tools to Universal Core without forwarding the ChatGPT token", async () => {
  ...
  assert.deepEqual(paths, ["/healthz", "/v1/icf/runtime/attestation", "/v1/runtime/hierarchy/evaluate", "/v1/work/preflight", "/v1/codex/context", "/v1/nira/branches", "/v1/research/plan", "/v1/research/validate", "/v1/nira/core-bridge", "/v1/action-evaluator"]);
  const callFor = (pathname) => calls.find((call) => new URL(call.url).pathname === pathname);
  assert(calls.filter((call) => !["/v1/work/preflight", "/v1/action-evaluator"].includes(new URL(call.url).pathname)).every((call) => call.init.headers.authorization === "Bearer tenant-a-key"));
  assert.equal(callFor("/v1/work/preflight").init.headers.authorization, `Bearer ${TENANT_GATEWAY_KEY}`);
  assert.ok(callFor("/v1/work/preflight").init.headers["x-sh-tenant-context"]);
  ...
  assert.equal(JSON.parse(callFor("/v1/runtime/hierarchy/evaluate").init.body).core_input.context.tenant_id, "tenant-a");
  const preflightBody = JSON.parse(callFor("/v1/work/preflight").init.body);
  assert.deepEqual(preflightBody.available_capabilities, ["github_connected_app"]);
  assert.equal(preflightBody.evidence_state.evidence_gap, true);
  assert.deepEqual(preflightBody.research_allowed_domains, ["docs.github.com"]);
  assert.deepEqual(preflightBody.host_native, {
    requested: false,
  ...
  assert.equal(JSON.stringify(preflightBody.host_native).includes("OPENAI_API_KEY"), false);
  for (const pathname of ["/v1/work/preflight", "/v1/codex/context", "/v1/research/plan", "/v1/research/validate", "/v1/nira/core-bridge"]) {
    assert.equal("domain_pack" in JSON.parse(callFor(pathname).init.body), false);
  }
  assert.deepEqual(JSON.parse(callFor("/v1/research/plan").init.body).allowed_domains, ["example.org"]);
  assert.equal(JSON.parse(callFor("/v1/research/validate").init.body).evidence_pack.question, "ricerca");
  assert.deepEqual(JSON.parse(callFor("/v1/nira/core-bridge").init.body).nyra_branches, ["context_intelligence"]);
  assert.equal(preflightBody.memory_context.tenant_id, "tenant-a");
  assert.equal(JSON.parse(callFor("/v1/nira/core-bridge").init.body).memory_context.revision, 7);
  assert.equal("core_runtime" in JSON.parse(callFor("/v1/nira/core-bridge").init.body), false);
...
test("Entity 360 SHADOW disable is owner-bound, tenant-gateway-only and hardcodes OFF", async () => {
  ...
  const owner = { tenantId: "codexai", kind: "codex", role: "owner_root",
    godMode: true, subject: "codex", ownerConfirmed: true };
  const result = await handlers.entity360ShadowDisableCoreRequest({
    expected_revision: 1,
    idempotency_key: "entity360-shadow-disable-a",
    mode: "SHADOW",
    enabled: true,
  }, owner);
  assert.equal(result.result.mode, "OFF");
  assert.equal(result.dedicated_core_gate.route, "entity_360_shadow_disable");
  assert.equal(calls.length, 1);
  assert.equal(calls[0].url.pathname, "/v1/entity-360/admin/feature-flag");
  assert.equal(calls[0].init.headers.authorization, `Bearer ${"g".repeat(48)}`);
  assert.ok(calls[0].init.headers["x-sh-tenant-context"]);
  assert.equal(calls[0].body.mode, "OFF");
  assert.equal(calls[0].body.enabled, false);
  assert.equal(calls[0].body.expected_revision, 1);
  assert.equal(calls[0].body.idempotency_key, "entity360-shadow-disable-a");
  assert.equal(calls[0].body.owner_confirmed, true);
  assert.match(calls[0].body.confirmation_reference, /^god_mode_codex:/u);
  assert.equal(calls[0].body.owner_context.owner_verified, true);
  await assert.rejects(() => handlers.entity360ShadowDisableCoreRequest({
    expected_revision: 2,
    idempotency_key: "entity360-shadow-disable-b",
  }, { ...owner, kind: "oauth", ownerConfirmed: false }), /owner_confirmation_required/u);
  assert.equal(calls.length, 1);
});
...
test("owner manual merge bridge forwards selectors only under an exact owner assertion", async () => {
  let call;
  const handlers = createCoreHandlers({
    universalCoreUrl: "https://core.test",
    universalCoreKeys: { "tenant-a": "tenant-a-key" },
  ...
  const identity = {
    tenantId: "tenant-a",
    kind: "oauth",
    subject: "auth0|manual-merge-owner",
    role: "tenant_owner",
    oauthOwnerElevated: true,
    ownerConfirmed: true,
    confirmationReference: "owner confirmed manual merge evidence readback",
  };
  await handlers.host_native_owner_manual_merge_readback({
    work_id: "14794fa6-2cdc-5f6a-8e68-211ff12c8cc6",
    intent_anchor_digest: "a".repeat(64),
    repository: "owner/repo",
    core_join_verdict_id: `hnj_${"b".repeat(40)}`,
    pull_request: 390,
    idempotency_key: "manual-merge-readback-pr-390",
    merged: true,
  ...
  assert.equal(body.confirmation_reference,
    "owner confirmed manual merge evidence readback");
  for (const forbidden of ["merged", "merge_commit", "head_commit", "health", "live_commit"]) {
...
test("owner manual merge bridge fails closed without an authenticated owner confirmation", async () => {
  ...
  await assert.rejects(handlers.host_native_owner_manual_merge_readback({
    work_id: "14794fa6-2cdc-5f6a-8e68-211ff12c8cc6",
    intent_anchor_digest: "a".repeat(64),
    repository: "owner/repo",
    core_join_verdict_id: `hnj_${"b".repeat(40)}`,
...
test("owner-confirmed manual merge finalizer projects Core evidence then uses normal Gallery closure", async () => {
  ...
  const receiptId = `hnmmr_${"1".repeat(40)}`;
  const authorization = {
    ...
    work_id: workId,
    action_ticket_id: "hnt_observe-12345678",
  ...
  const handlers = createCoreHandlers({
    ...
    fetchImpl: async (url, init) => {
      ...
      return new Response(JSON.stringify({
        ...
        finalize_authorization_proof: authorizationProof,
      }), { status: 200, headers: { "content-type": "application/json" } });
    },
    tenantWorkGallery: {
      async recordOwnerManualMergeReleaseEvidence(identity, source) {
        galleryCalls.push({ method: "evidence", identity, source });
        return {
          adapter: "software_git",
          evidence_id: "4b664e7c-f4cc-4c37-962b-97856eef650a",
          evidence_digest: "4".repeat(64),
          legacy_bridged: true,
        };
      },
      async finalizeGenericClosure(identity, input) {
  ...
  assert.equal(calls.length, 1);
  assert.equal(galleryCalls[0].method, "evidence");
  assert.equal(Object.hasOwn(galleryCalls[0].source, "server_owned"), false);
  assert.deepEqual(galleryCalls[0].source.finalize_authorization, authorization);
  assert.deepEqual(galleryCalls[0].source.finalize_authorization_proof,
    authorizationProof);
  assert.equal(galleryCalls[1].method, "closure");
  assert.deepEqual(galleryCalls[1].input, {
    work_id: workId,
    adapter: "software_git",
  });
