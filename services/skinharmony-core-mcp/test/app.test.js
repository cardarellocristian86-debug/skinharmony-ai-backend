...
test("Nyra assignment claim and submit preserve the Work target and idempotency key at the Core gate", () => {
  const serverSource = fs.readFileSync(new URL("../src/server.js", import.meta.url), "utf8");
  for (const [handler, action] of [
    ...
    const end = serverSource.indexOf("const result = await nyraAutopilotRuntime", start);
    assert.ok(start >= 0, handler);
    assert.ok(end > start, handler);
    const gate = serverSource.slice(start, end);
    assert.match(gate, new RegExp(`tenantWorkCoordinationActionType\\(\\"${handler}\\"\\)`));
    assert.match(gate, new RegExp(`tenantWorkCoordinationTarget\\(\\"${handler}\\", args\\)`));
    assert.match(gate, /args\.idempotency_key/);
    assert.equal(tenantWorkCoordinationActionType(handler), action);
  }
});
