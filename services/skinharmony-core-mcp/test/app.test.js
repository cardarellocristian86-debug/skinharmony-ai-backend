...
test("keeps Codex bearer compatibility and exposes MCP security schemes", async () => serve(async (base) => {
  ...
  assert(body.result.tools.every((tool) => tool.outputSchema));
  assert(body.result.tools.every((tool) => tool.description.startsWith("Use this when")));
  for (const name of ["software_intelligence", "software_job_status", "software_evidence_get", "skin_analyzer", "scalp_analyzer"]) {
    assert(body.result.tools.find((tool) => tool.name === name), `missing new tool ${name}`);
  }
}));
...
test("journals successful and failed tool calls and returns safe MCP tool errors", async () => {
  const events = [];
...
test("rejects unknown and cross-tenant arguments before handler execution", async () => {
  let called = false;
  const app = createApp(config, {
    handlers: { core_health: async () => { called = true; return { structuredContent: { ok: true }, content: [] }; } },
  });
  ...
  try {
    const response = await fetch(`http://127.0.0.1:${server.address().port}/mcp`, {
      method: "POST",
      headers: { authorization: "Bearer codex-key", "content-type": "application/json" },
      body: JSON.stringify({ jsonrpc: "2.0", id: 60, method: "tools/call", params: { name: "core_health", arguments: { tenant_id: "other-tenant" } } }),
    });
    const body = await response.json();
    assert.equal(body.error.code, -32602);
    assert.equal(body.error.data.violations[0].code, "additional_property");
