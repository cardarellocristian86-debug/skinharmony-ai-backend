...
function makeHandlers(calls) {
  return createCoreHandlers({
    ...
    fetchImpl: async (url, init) => {
      calls.push({ url, init });
      return response({ ok: true, tenant_id: "tenant-owner-a", run: { run_id: "run_01", status: "running" } });
    },
  });
}
...
test("only a confirmed tenant OAuth provider owner can start the bounded OpenAI run", async () => {
  ...
  assert.deepEqual(Object.keys(body).sort(), ["confirmation_reference", "owner_confirmed", "owner_context", "task", "tenant_id"]);
